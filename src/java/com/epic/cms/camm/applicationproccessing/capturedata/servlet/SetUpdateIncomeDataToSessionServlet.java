 ...
 * @author janaka_h
 */
public class SetUpdateIncomeDataToSessionServlet extends HttpServlet {
...
//
            ...
            incomeBean.setIncomeCatogaryCode(request.getParameter("incomeCategory"));
            incomeBean.setAmount(request.getParameter("amount"));

            if (incomeBean.getIncomeCatogaryCode().isEmpty()) {
                request.setAttribute("invalidCategory", "Requierd");
                msg = 1;
            } else {

                if (sessionVarlist.getSessionIncomeList() != null) {
                    for (int x = 0; x < sessionVarlist.getSessionIncomeList().size(); x++) {

                        if (incomeBean.getIncomeCatogaryCode().equals(sessionVarlist.getSessionIncomeList().get(x).getIncomeCatogaryCode())) {
                            request.setAttribute("invalidCategory", "Exist");
                            msg = 1;
                        }
            ...
            if (incomeBean.getAmount().isEmpty()) {
                request.setAttribute("invalidAmount", "Requierd");
                msg = 1;
            } else if (!validObject.isNumeric(incomeBean.getAmount())) {
                request.setAttribute("invalidAmount", "Invalid");
                msg = 1;
            }
...
//
            if (msg == 0) {
                //new by chinthaka
                List<IncomeTypeBean> list1 = new ArrayList<IncomeTypeBean>();
                list1 = sessionVarlist.getIncomeTypeList();
                for (int k = 0; k < list1.size(); k++) {
                    if (incomeBean.getIncomeCatogaryCode().equals(list1.get(k).getIncomeCode())) {
                        incomeBean.setIncomeCatogary(list1.get(k).getDescription());
                        break;
                    }
                }
                ////
                if (sessionVarlist.getSessionIncomeList() != null) {
                    sessionVarlist.getSessionIncomeList().add(incomeBean);
                    ...
                    list.add(incomeBean);
                    sessionVarlist.setSessionIncomeList(list);
...
//            sessionObj.setAttribute(SessionVarName.SESSION_OBJ, sessionVarlist);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(SetUpdateIncomeDataToSessionServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

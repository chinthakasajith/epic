 ...
 * @author asela
 */
public class CorporateAssignPersistance {
    ...
    public boolean insertApplicationHistory(Connection con, ApplicationHistoryBean historyBean) throws Exception {
        ...
        if (historyBean != null) {
            try {
                ...
                System.out.println("######## " + historyBean.getApplicationId() + " *** " + historyBean.getApplicationLevel() + " *** " + historyBean.getStatus() + " *** " + historyBean.getRemarks() + " ** " + historyBean.getLastUpdatedUser());
    ...
    public boolean insertAssignApplication(Connection con, ApplicationAssignBean assignBean) throws Exception {
        ...
        try {
            insertStat = con.prepareStatement("INSERT INTO CARDAPPLICATION(APPLICATIONID,CARDCATEGORY,IDENTIFICATIONTYPE, "
                    ...
                    + "LASTUPDATEDUSER,CARDAPPLIACTIONDOMAIN) "
                    + "VALUES(?,?,?,?,?,?,?,?,?,?,?) ");
    ...
    public List<ApplicationAssignBean> generalSearch(Connection con, SearchAssignAppBean searchBean) throws Exception {
        ...
        try {
            ...
            if (!condition.equals("")) {
                strSqlBasic += " WHERE a.CARDAPPLIACTIONDOMAIN = AD.DOMAINID AND CS.APPLICATIONID = a.APPLICATIONID AND a.ASSIGNSTATUS = st.STATUSCODE AND a.PRIORITYLEVELCODE = pl.PRIORITYLEVELCODE  AND (bk.BANKCODE in (select BANKCODE from COMMONPARAMETER) OR  a.branchcode is null) AND a.CARDCATEGORY = CC.CATEGORYCODE AND CS.APPLICATIONSTATUS IN ('" + StatusVarList.APP_INITIATE + "','" + StatusVarList.APP_PROCESS + "') AND " + condition + " AND a.CARDCATEGORY IN ('" + StatusVarList.ESTABLISHMENT_CAT + "','" + StatusVarList.CORPORATE_CAT + "') ORDER BY a.APPLICATIONID";
            } else {
                strSqlBasic += " WHERE a.CARDAPPLIACTIONDOMAIN = AD.DOMAINID AND CS.APPLICATIONID = a.APPLICATIONID AND a.ASSIGNSTATUS = st.STATUSCODE AND a.PRIORITYLEVELCODE = pl.PRIORITYLEVELCODE  AND (bk.BANKCODE in (select BANKCODE from COMMONPARAMETER) OR  a.branchcode is null) AND a.CARDCATEGORY = CC.CATEGORYCODE AND CS.APPLICATIONSTATUS IN ('" + StatusVarList.APP_INITIATE + "','" + StatusVarList.APP_PROCESS + "')  AND a.CARDCATEGORY IN ('" + StatusVarList.ESTABLISHMENT_CAT + "','" + StatusVarList.CORPORATE_CAT + "') ORDER BY a.APPLICATIONID";
            }
...
//                    + "a.ASSIGNUSER, a.ASSIGNSTATUS , st.DESCRIPTION ,a.LASTUPDATEDUSER,a.LASTUPDATEDTIME,a.CREATETIME "
//                    + "FROM CARDAPPLICATION a ,STATUS st,PRIORITYLEVEL pl ,BANKBRANCH bk,CARDCATEGORY CC "
//                    + "WHERE a.ASSIGNSTATUS = st.STATUSCODE AND a.PRIORITYLEVELCODE = pl.PRIORITYLEVELCODE AND "
//                    + "a.BRANCHCODE = bk.BRANCHCODE(+) AND bk.BANKCODE =(select BANKCODE from commonparameter) AND a.CARDCATEGORY = CC.CATEGORYCODE AND a.IDENTIFICATIONNO=? ORDER BY a.APPLICATIONID";
//
//
//            stmt = con.prepareStatement(strSqlBasic);

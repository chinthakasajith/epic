 ...
 * @author nalin
 */
public class UpdateMerchantLocationAssignDataServlet extends HttpServlet {
                            ...
                            if (successInsert) {
                                if (sessionVarlist.getManualTxnStatus().equals("NO")) {
                                    if (this.checkExsistanceOfManualTerminal(merchantLocBean.getMerchantId())) {
                                        this.deleteManualTerminal(merchantLocBean.getMerchantId());
                                    }
                                    ...
                                    sessionVarlist.setMerchantId(merchantLocBean.getMerchantId());
                                    if (this.checkExsistanceOfManualTerminal(merchantLocBean.getMerchantId())) {
                                        request.setAttribute("operationtype", "UPDATE");
                                        rd = getServletContext().getRequestDispatcher("/LoadUpdateMerchantLocationManualTxnServlet");
                                    } else {
...
//            merchantLocBean.setPostalCode(postalCode);
    ...
    private boolean deleteManualTerminal(String mId) throws Exception {
        boolean success = false;

        try {

            merchantLocManager = new MerchantLocationManager();
            success = merchantLocManager.deleteManualTerminal(mId);

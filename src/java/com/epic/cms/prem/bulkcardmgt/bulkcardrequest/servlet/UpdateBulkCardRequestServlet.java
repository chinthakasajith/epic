 ...
 * @author nisansala
 */
public class UpdateBulkCardRequestServlet extends HttpServlet {
                ...
                if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                    ...
                    try {
                        ...
                        if (validateUserInput(bulkCdReqbean)) {
                            ...
                            try {
                                if (request.getParameter("batchStatus").equals("BCRP")) {
                                    successUpdate = updateBulkCdReq(bulkCdReqbean);

                                    if (successUpdate) {
                                        request.setAttribute("successMsg", MessageVarList.SECURITY_QUES_MGT_QUESTION_NO_SUCCESS_UPDATE + " Batch ID : " +bulkCdReqbean.getBatchID());
                                        rd = getServletContext().getRequestDispatcher("/LoadBulkCardRequestServlet");
                                        rd.include(request, response);
                                    }
                                } else {

                                    request.setAttribute("errorMsg", MessageVarList.NO_UPDATE);
                                    rd = getServletContext().getRequestDispatcher("/LoadBulkCardRequestServlet");
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update Bulk Card Request. Batch Id: '" + request.getParameter("batchId") + "' by: " + sessionVarlist.getCMSSessionUser().getUserName());//******************
    ...
    public Boolean updateBulkCdReq(BulkCardRequestBean bean) throws Exception {
        boolean success = false;
        try {
            bulkCdReqMgr = new BulkCardRequestManager();
            success = bulkCdReqMgr.updateBulkCdReq(bean, systemAuditBean);

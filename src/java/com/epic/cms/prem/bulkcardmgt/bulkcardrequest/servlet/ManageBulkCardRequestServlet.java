 ...
 * @author nisansala
 */
public class ManageBulkCardRequestServlet extends HttpServlet {
            ...
            try {
                ...
                if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                    ...
                    try {
                        ...
                        if (validateUserInput(bulkCdReqbean)) {

                            this.setAudittraceValue(request);

                            try {
                                String bulkStatus = request.getParameter("confirmStatus");
                                //insert the user given values to the MCC table
                                successConfirm = confirmReceived(bulkCdReqbean);
                                if (successConfirm) {
                                    if (bulkStatus.equals("BCRF")) {
                                        request.setAttribute("successMsg", MessageVarList.SUCCESS_CONFIRM);
                                    } else if (bulkStatus.equals("BCDR")) {
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update: '" + request.getParameter("batchId") + "' Bulk Card Batch Id by: " + sessionVarlist.getCMSSessionUser().getUserName());//******************
    ...
    public boolean validateUserInput(BulkCardRequestBean bulkReq) throws Exception {
        boolean isValidate = true;


        try {
            if (bulkReq.getRemark().contentEquals("")) {
                isValidate = false;
    ...
    public Boolean setUserInputToBean(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            bulkCdReqbean.setRemark(request.getParameter("remarks").trim());
            bulkCdReqbean.setReceivedUser(sessionUser.getUserName());
            bulkCdReqbean.setLastUpdatedUser(sessionUser.getUserName());
            bulkCdReqbean.setStatus(request.getParameter("confirmStatus"));
    ...
    public Boolean confirmReceived(BulkCardRequestBean bean) throws Exception {
        boolean success = false;
        try {
            bulkCdReqMgr = new BulkCardRequestManager();
            success = bulkCdReqMgr.confirmReceived(bean, systemAuditBean);

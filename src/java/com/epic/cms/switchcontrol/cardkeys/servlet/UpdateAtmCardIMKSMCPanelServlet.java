 ...
 * @author asela
 */
public class UpdateAtmCardIMKSMCPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String imksmc = request.getParameter("imksmc");    
            
            if (imksmc.length() == 32) {
                if (UserInputValidator.isHexxx(imksmc)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.IMKSMC_PREFIX + imksmc).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setImksmc(imksmc);
                        keyBean.setImksmcKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateIMKSMCKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update IMKSMC and IMKSMC KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateIMKSMCKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateIMKSMCKeyDetails(keyBean, systemAuditBean);

 ...
 * @author asela
 */
public class UpdateAtmCardCVK1BPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String cvk1b = request.getParameter("cvk1b");    
            
            if (cvk1b.length() == 16) {
                if (UserInputValidator.isHexxx(cvk1b)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.CVK_PREFIX + cvk1b).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setCvk1b(cvk1b);
                        keyBean.setCvk1bKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateCVK1BKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CVK1B and CVK1B KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateCVK1BKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateCVK1BKeyDetails(keyBean, systemAuditBean);

 ...
 * @author asela
 */
public class UpdateAtmCardCVK2APanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String cvk2a = request.getParameter("cvk2a");    
            
            if (cvk2a.length() == 16) {
                if (UserInputValidator.isHexxx(cvk2a)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.CVK_PREFIX + cvk2a).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setCvk2a(cvk2a);
                        keyBean.setCvk2aKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateCVK2AKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CVK2A and CVK2A KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateCVK2AKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateCVK2AKeyDetails(keyBean, systemAuditBean);

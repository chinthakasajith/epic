 ...
 * @author asela
 */
public class UpdateAtmCardCVK1PanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String cvk1 = request.getParameter("cvk1");    
            
            if (cvk1.length() == 32) {
                if (UserInputValidator.isHexxx(cvk1)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.CVK_PREFIX + cvk1).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setCvk1(cvk1);
                        keyBean.setCvk1KVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateCVK1KeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    public boolean updateCVK1KeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateCVK1KeyDetails(keyBean, systemAuditBean);

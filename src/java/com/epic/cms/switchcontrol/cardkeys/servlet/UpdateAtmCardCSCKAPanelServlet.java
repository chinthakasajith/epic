 ...
 * @author asela
 */
public class UpdateAtmCardCSCKAPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String cscka = request.getParameter("cscka");    
            
            if (cscka.length() == 16) {
                if (UserInputValidator.isHexxx(cscka)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.CSCK_PREFIX + cscka).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setCscka(cscka);
                        keyBean.setCsckaKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateCSCKAKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CSCKA and CSCKA KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateCSCKAKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateCSCKAKeyDetails(keyBean, systemAuditBean);

 ...
 * @author asela
 */
public class UpdateAtmCardCVK1APanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String cvk1a = request.getParameter("cvk1a");    
            
            if (cvk1a.length() == 16) {
                if (UserInputValidator.isHexxx(cvk1a)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.CVK_PREFIX + cvk1a).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setCvk1a(cvk1a);
                        keyBean.setCvk1aKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateCVK1AKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CVK1A and CVK1A KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateCVK1AKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateCVK1AKeyDetails(keyBean, systemAuditBean);

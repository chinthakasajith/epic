 ...
 * @author asela
 */
public class UpdateAtmCardIMKACPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String imkac = request.getParameter("imkac");    
            
            if (imkac.length() == 32) {
                if (UserInputValidator.isHexxx(imkac)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.IMK_AC_PREFIX + imkac).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setImkac(imkac);
                        keyBean.setImkacKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateIMKACKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update IMKAC and IMKAC KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateIMKACKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateIMKACKeyDetails(keyBean, systemAuditBean);

 ...
 * @author asela
 */
public class UpdateAtmCardCVK2PanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String cvk2 = request.getParameter("cvk2");    
            
            if (cvk2.length() == 32) {
                if (UserInputValidator.isHexxx(cvk2)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.CVK_PREFIX + cvk2).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setCvk2(cvk2);
                        keyBean.setCvk2KVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateCVK2KeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CVK2 and CVK2 KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateCVK2KeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateCVK2KeyDetails(keyBean, systemAuditBean);

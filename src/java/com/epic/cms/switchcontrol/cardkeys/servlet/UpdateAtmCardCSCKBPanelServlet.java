 ...
 * @author asela
 */
public class UpdateAtmCardCSCKBPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String csckb = request.getParameter("csckb");    
            
            if (csckb.length() == 16) {
                if (UserInputValidator.isHexxx(csckb)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.CSCK_PREFIX + csckb).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setCsckb(csckb);
                        keyBean.setCsckbKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateCSCKBKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CSCKB and CSCKB KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateCSCKBKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateCSCKBKeyDetails(keyBean, systemAuditBean);

 ...
 * @author asela
 */
public class UpdateAtmCardPVKBPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String pvkb = request.getParameter("pvkb");    
            
            if (pvkb.length() == 16) {
                if (UserInputValidator.isHexxx(pvkb)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.PVK_PREFIX + pvkb).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setPvkb(pvkb);
                        keyBean.setPvkbKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updatePVKBKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update PVKB and PVKB KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updatePVKBKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updatePVKBKeyDetails(keyBean, systemAuditBean);

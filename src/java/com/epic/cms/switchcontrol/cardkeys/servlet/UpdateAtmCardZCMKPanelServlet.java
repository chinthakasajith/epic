 ...
 * @author asela
 */
public class UpdateAtmCardZCMKPanelServlet extends HttpServlet {
            ...
            if (zcmk.length() == 32) {
                if (UserInputValidator.isHexxx(zcmk)) {
                    ...
                    try {
                        ...
                        keyBean.setZcmk(zcmk);
                        keyBean.setZcmkKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateZCMKKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update ZCMK and ZCMK KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateZCMKKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateZCMKKeyDetails(keyBean, systemAuditBean);

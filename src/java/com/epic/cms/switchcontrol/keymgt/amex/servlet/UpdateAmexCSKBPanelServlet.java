 ...
 * @author mahesh_m
 */
public class UpdateAmexCSKBPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String cskb = request.getParameter("cskb");

            if (cskb.length() == 16) {
                if (UserInputValidator.isHexxx(cskb)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.CSCK_PREFIX + cskb).substring(4);
                        ...
                        if (chanelId == null) {
                            ...
                            if (keyMgtmanager.isChanelAvailable(chanelId)) {
                                keyMgtmanager.updateCSKB(cskb, KVC, chanelId, systemAuditBean);
                            } else {
                                keyMgtmanager.addCSKB(cskb, KVC, chanelId, systemAuditBean);
                            }
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update AMEX CSKB and KVC by " + sessionVarlist.getCMSSessionUser().getUserName());

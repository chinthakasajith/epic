 ...
 * @author asela
 */
public class UpdateCreditProprietaryZCMKPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String zcmk = request.getParameter("zcmk");

            if (zcmk.length() == 32) {
                if (UserInputValidator.isHexxx(zcmk)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.ZCMK_PREFIX + zcmk).substring(4);
                        ...
                        if (chanelId == null) {
                            ...
                            if (keyMgtmanager.isChanelAvailable(chanelId)) {
                                keyMgtmanager.updatezcmk(zcmk, KVC, chanelId, systemAuditBean);
                            } else {
                                keyMgtmanager.addzcmk(zcmk, KVC, chanelId, systemAuditBean);
                            }
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CIRRUS ZCMK and KVC by " + sessionVarlist.getCMSSessionUser().getUserName());

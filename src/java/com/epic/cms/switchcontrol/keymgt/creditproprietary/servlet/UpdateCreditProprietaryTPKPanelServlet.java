 ...
 * @author asela
 */
public class UpdateCreditProprietaryTPKPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (tpk.length() == 32) {
                if (UserInputValidator.isHexxx(tpk)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.AWK_PREFIX + tpk).substring(4);
                       ...
                       String chanelId = keyMgtmanager.getChanelId(StatusVarList.CHANEL_TYPE_CREADIT_PROPR);
                        ...
                        if (chanelId == null) {
                            ...
                            if (keyMgtmanager.isChanelAvailable(chanelId)) {
                                keyMgtmanager.updateTPK(tpk, KVC, chanelId, systemAuditBean);
                            } else {
                                keyMgtmanager.addTPK(tpk, KVC, chanelId, systemAuditBean);
                            }
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CIRRUS TPK and TPK KVC by " + sessionVarlist.getCMSSessionUser().getUserName());

 ...
 * @author asela
 */
public class UpdateCreditProprietaryIWKPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String iwk = request.getParameter("iwk");

            if (iwk.length() == 32) {
                if (UserInputValidator.isHexxx(iwk)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.IWK_PREFIX + iwk).substring(4);
                        ...
                        if (chanelId == null) {
                            ...
                            if (keyMgtmanager.isChanelAvailable(chanelId)) {
                                keyMgtmanager.updateIWK(iwk, KVC, chanelId, systemAuditBean);
                            } else {
                                keyMgtmanager.addIWK(iwk, KVC, chanelId, systemAuditBean);
                            }
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CIRRUS IWK and IWK KVC by " + sessionVarlist.getCMSSessionUser().getUserName());

 ...
 * @author asela
 */
public class UpdateCreditProprietaryPVVBPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String pvvb = request.getParameter("pvvb");

            if (pvvb.length() == 16) {
                if (UserInputValidator.isHexxx(pvvb)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.PVVA_B_PREFIX + pvvb).substring(4);
                        ...
                        if (chanelId == null) {
                            ...
                            if (keyMgtmanager.isChanelAvailable(chanelId)) {
                                keyMgtmanager.updateVISAPVVB(pvvb, KVC, chanelId, systemAuditBean);
                            } else {
                                keyMgtmanager.addVISAPVVB(pvvb, KVC, chanelId, systemAuditBean);
                            }
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CIRRUS PVVB and PVVB KVC by " + sessionVarlist.getCMSSessionUser().getUserName());

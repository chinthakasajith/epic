 ...
 * @author asela
 */
public class UpdateCreditProprietaryCVVBPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String cvvb = request.getParameter("cvvb");

            if (cvvb.length() == 16) {
                if (UserInputValidator.isHexxx(cvvb)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.CVVA_B_PREFIX + cvvb).substring(4);
                        ...
                        if (chanelId == null) {
                            ...
                            if (keyMgtmanager.isChanelAvailable(chanelId)) {
                                keyMgtmanager.updateVISACVVB(cvvb, KVC, chanelId, systemAuditBean);
                            } else {
                                 keyMgtmanager.addVISACVVB(cvvb, KVC, chanelId, systemAuditBean);
                            }
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CIRRUS CVVB and CVVB KVC by " + sessionVarlist.getCMSSessionUser().getUserName());

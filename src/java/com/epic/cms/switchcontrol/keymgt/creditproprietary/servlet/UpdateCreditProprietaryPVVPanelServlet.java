 ...
 * @author asela
 */
public class UpdateCreditProprietaryPVVPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String pvv = request.getParameter("pvv");

            if (pvv.length() == 32) {
                if (UserInputValidator.isHexxx(pvv)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        KVC = securityModule.getServerMsg(StatusVarList.PVVA_B_PREFIX + pvv).substring(4);
                        ...
                        if (chanelId == null) {
                            ...
                            if (keyMgtmanager.isChanelAvailable(chanelId)) {
                                keyMgtmanager.updatePVV(pvv, KVC, chanelId, systemAuditBean);
                            } else {
                                keyMgtmanager.addPVV(pvv, KVC, chanelId, systemAuditBean);
                            }
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update MASTER PVV and PVV KVC by " + sessionVarlist.getCMSSessionUser().getUserName());

 ...
 * @author mahesh_m
 */
public class UpdateSWTPKPanelServlet extends HttpServlet {
        ...
        try {
            ...
            if (tpk.length() == 32) {
                if (UserInputValidator.isHexxx(tpk)) {
                    ...
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.PVVA_B_PREFIX + tpk).substring(4);
                        keyMgtmanager = new KeyManagementManager();
                         this.setAudittraceValue(request);
                         
                        if (keyMgtmanager.isOnlineKeyAvailable(StatusVarList.SWITCH_KEY_ID)) {
                            keyMgtmanager.updateTPK(tpk, KVC, StatusVarList.SWITCH_KEY_ID,systemAuditBean);
                        } else {
                            keyMgtmanager.addTPK(tpk, KVC, StatusVarList.SWITCH_KEY_ID,systemAuditBean);
                        }
...
//                rd.forward(request, response);
     ...
     private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update SWITCH POS TPK and TPK KVC by " + sessionVarlist.getCMSSessionUser().getUserName());

 ...
 * @author chanuka
 */
public class ProcessGenerateVisaCardServlet extends HttpServlet {
    ...
    private String cardsOfProduct = "", cardType = "", cardTypeDes = "";
        ...
        try {
            ...
            try {
                ...
                if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                    ...
                    cards = request.getParameter("visacards");
                    ...
                    if (cards == null || cards.isEmpty()) {
                        ...
                        // get the relavent emboss file format string
                        this.getVisaEmbossingFormat(cardType);
                        // get all card product related to card type
                        this.getAllCardProduct(cardType);
                        // get all emboss fields
                        this.getEmbossFields();


                        // get the date to make the generated file name
                        ...
                        if (embossFileFormat != null) {
                            ...
                            while (itr.hasNext()) {
                                ...
                                // get all the card list related to selected cards ,card type and card product
                                List<CardEmbossingCardDetailsBean> visaProductCardList = this.getAllVISACardProductToEmboss(me.getKey().toString(), cardType);

                                for (int i = 0; i < visaProductCardList.size(); i++) {


                                    if (i == (visaProductCardList.size() - 1)) {

                                        cardsOfProduct += visaProductCardList.get(i).getCardNumber();
                                    ...
                                    for (int j = 0; j < formatedRecord.length; j++) {
                                        ...
                                        if (j % 2 == 0) {
                                            ...
                                            if (!embossDetailBean.getMatchedTable().equals("NO")) {
                                                ...
                                                valueFromTable = this.getTableFieldValue(embossDetailBean.getMatchedTable(), embossDetailBean.getMatchedField(), visaProductCardList.get(i).getCardNumber());
...
//            out.close();
    ...
    private List<CardEmbossingCardDetailsBean> getAllVISACardProductToEmboss(String cardProduct, String cardType) throws Exception {
        try {
            cardEmbossingManager = new CardEmbossingMgtManager();
            cardEmbossingVISAList = cardEmbossingManager.getAllCardProductToEmbossByCardType(cards, cardProduct, cardType);

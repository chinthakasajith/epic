 ...
 * @author mahesh_m
 */
public class generateDebitPinServlet extends HttpServlet {
...
//    private ChanelKeysBean keyDetails;
    ...
    private String chanelId = null;
    private ApplicationHistoryBean historybean = null;
    HttpSession sessionObj;
    private String url = "/cpmm/pingenarateandmail/debitcardpingeneration.jsp";
...
//            chanelId = keyMgtmanager.getChanelId(StatusVarList.CHANEL_TYPE_VISA);
...
//                        Boolean onlineUpdateStatus = updateOnleCard(ePPKPin, PVV, OFFSET, pinMethod, CVV, cardNumber);
                        ...
                        if (CardUpdateStatus) {
                            ...
                            request.setAttribute("successMsg", MessageVarList.SUCCES_PIN_AND_PVC_CVV + cardNumber);
                        } else {
                            request.setAttribute("errorMsg", MessageVarList.DBCONNECTION_ERROR);
                        }

                    } else {
                        request.setAttribute("errorMsg", MessageVarList.ERROR_CARDNO_NOT_IN_ONLINE_DB);
                    }
                } else {
                    request.setAttribute("errorMsg", MessageVarList.ERROR_CVV_GEN);
                }

            } else {
                request.setAttribute("errorMsg", MessageVarList.ERROR_PVV_GEN);
            }
...
//            String CVVData = ANB + expDate;
    ...
    private void setAudittraceValue(HttpServletRequest request, String cardNumber) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Generated Debit Pin Card Number: " + cardNumber + sessionVarlist.getCMSSessionUser().getUserName());

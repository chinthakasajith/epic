 ...
 * @author mahesh_m
 */
public class SearchPinMailServlet extends HttpServlet {
    ...
    private String url = "/cpmm/pingenarateandmail/pinmail.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try {
                ...
                String pageCode = PageVarList.PINMAIL;
                ...
                if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                    ...
                    if (validateInputs(searchBean)) {
                        ...
                        this.getAllPinMailList();


                        for (int idx = 0; idx < pingMailList.size(); idx++) {
                            masterCardsToEmbossList.add(pingMailList.get(idx).getCardNumber());


                            pingMailList.get(idx).setCardNumberHidden("************".concat(pingMailList.get(idx).getCardNumber().substring(12, pingMailList.get(idx).getCardNumber().length())));


                        }


                        request.setAttribute("pingMailList", pingMailList);
...
//            searchBean.setEmbossType(embossType);
    ...
    private void getAllPinMailList() throws Exception {
        try {

            pinAndmailManager = new PinGenerationAndmailManager();
            pingMailList = pinAndmailManager.getAllPinMailList(searchBean, DataTypeVarList.CARD_DOMAIN_CREDIT);

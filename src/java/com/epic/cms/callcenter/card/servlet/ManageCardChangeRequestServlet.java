...
import com.epic.cms.callcenter.card.businesslogic.CardChangeRequestManager;
 ...
 * @author nisansala
 */
public class ManageCardChangeRequestServlet extends HttpServlet {
    ...
    private CardChangeRequestManager cdChngMgr;
    ...
    private HashMap<String, String> cardProduct = null;
    ...
    private String url = "/callcenter/card/cardchangerequesthome.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try {
                request.setAttribute("operationType", "cardchange");
                ...
                try {
                    ...
                    String pageCode = PageVarList.CARD_CHANGE_REQ;
                    ...
                    if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                        try {
                            ...
                            if (reasonCode.equals("TYPE")) {
                                request.setAttribute("typeEnable", "yes");
                            } else {
                                request.setAttribute("typeEnable", "no");
                            }
                            ...
                            //get card products
                            this.loadCardProduct(card.getNewCardType());
                            request.setAttribute("cardProduct", cardProduct);
                            ...
                            if (validateUserInput(card, reasonCode)) {
                                ...
                                //*************************************
                                cdChngMgr = new CardChangeRequestManager();
                                success = cdChngMgr.changeCard(reasonCode, card, callerLogID, systemAuditBean);
                                //**************************************

                                if (success == 1) {
                                    request.setAttribute("successMsg", MessageVarList.SUCCESS_CHANGE_REQUEST + " for Card " + card.getCardNumber());
                                    ...
                                    request.setAttribute("reason", reasonCode);
                                    request.setAttribute("errorMsg", "A request has set for the card number");    
                                    request.setAttribute("cardBean", card);
    ...
    public Boolean setUserInputToBean(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            card.setCardProduct(request.getParameter("cardproduct"));
            card.setCardProdDes(request.getParameter("cardproductdes"));
            ...
            if (request.getParameter("reason").equals("PRODUCT")) {
                card.setNewCardType(request.getParameter("cardtype"));
            } else {
                card.setNewCardType(request.getParameter("newcardtype"));
            }
            card.setNewCardProduct(request.getParameter("newcardproduct"));
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Request Replace " + " Card No = " + card.getCardNumber() + " in Card Request Management by " + sessionVarlist.getCMSSessionUser().getUserName());
            ...
            systemAuditBean.setPageCode(PageVarList.CARD_CHANGE_REQ);
    ...
    public boolean validateUserInput(CardBean card, String reasonCode) throws Exception {
        ...
        try {
            if (reasonCode.equals("TYPE")) {
                if (card.getNewCardType().contentEquals("") || card.getNewCardType().length() <= 0) {
                    isValidate = false;
                    errorMessage = MessageVarList.RENEW_CONFIRM_EMPTY;
                }
            } else if (card.getNewCardProduct().contentEquals("") || card.getNewCardProduct().length() <= 0) {
                isValidate = false;
                errorMessage = MessageVarList.CREDITSCORE_RECOMMEND_CARDPRODUCT_EMPTY;
            } else if (card.getPriorityLevel().contentEquals("") || card.getPriorityLevel().length() <= 0) {
                isValidate = false;
            ...
            } else if (!UserInputValidator.isDescription(card.getRemark())) {
                isValidate = false;
                errorMessage = MessageVarList.REMARKS_INVALID;
            }
        } catch (Exception ex) {
            isValidate = false;
            throw ex;
    ...
    private void loadCardProduct(String cardType) throws Exception {
        cdChngMgr = new CardChangeRequestManager();
        cardProduct = cdChngMgr.loadCardProduct(cardType);
    }

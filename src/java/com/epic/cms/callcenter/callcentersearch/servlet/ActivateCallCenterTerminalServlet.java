 ...
 * @author nalin
 */
public class ActivateCallCenterTerminalServlet extends HttpServlet {
    ...
    //private String url = "/aquirecallcenter/terminalactivation.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try {
                ...
                String pageCode = PageVarList.CALLCENTER_TERMINAL_ACTIVATION;
                ...
                if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                    ...
                    if (isTerminalActive(terminalId)) {
                        ...
                        request.setAttribute("errorMsg", MessageVarList.TERMINAL_ALREADY_ACTIVATED);
                        ...
                        if (isTerminalAllocated(terminalId)) {//terminal Id previously not allocated
                            ...
                            request.setAttribute("errorMsg", MessageVarList.CANTACTIVATE);
                            rd = getServletContext().getRequestDispatcher(backUrl + terminalId + "&mid=" + merchantId);
                        } else {//prevoiusly allocated
                            this.setAudittraceValue(request);
                            if (updateTerminalStatusToActivate(terminalId)) {
                                request.setAttribute("successMsg", MessageVarList.TERMINAL_SUCSESS_ACTIVATED);
                                rd = getServletContext().getRequestDispatcher(backUrl + terminalId + "&mid=" + merchantId);
                            } else {
                                request.setAttribute("errorMsg", MessageVarList.TERMINAL_ACTIVATION_FAIL);
                                rd = getServletContext().getRequestDispatcher(backUrl + terminalId + "&mid=" + merchantId);
                            }
                        }
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Activated Terminal." + " Terminal ID : " + terminalId + "; by :" + sessionVarlist.getCMSSessionUser().getUserName());
            ...
            systemAuditBean.setPageCode(PageVarList.CALLCENTER_TERMINAL_ACTIVATION);
    ...
    public Boolean updateTerminalStatusToActivate(String terminalID) throws Exception {
        Boolean status = false;
        try {
            status = allocationmanager.updateTerminalStatusToActivate(terminalID, systemAuditBean, StatusVarList.ACTIVE_STATUS, "1");
        } catch (Exception e) {
            throw e;
        }
        return status;

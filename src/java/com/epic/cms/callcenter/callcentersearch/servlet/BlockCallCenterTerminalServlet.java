 ...
 * @author nalin
 */
public class BlockCallCenterTerminalServlet extends HttpServlet {
    ...
    //private String url = "/aquirecallcenter/terminalblock.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try {
                ...
                String pageCode = PageVarList.CALLCENTER_TERMINAL_BLOCK;
                    ...
                    if (!this.isTransactionsSettled(merchantId, terminalId)) {
                        ...
                        if (isTerminalActive(terminalId)) {
                            if (isTerminalAllocated(terminalId)) {//terminal Id previously not allocated
                                ...
                                request.setAttribute("errorMsg", MessageVarList.CANT_BLOCK_NOTALLOCATED);
                                ...
                                if (updateTerminalStatus(terminalId)) {
                                    ...
                                    request.setAttribute("successMsg", MessageVarList.TERMINAL_SUCSESS_BLOCK);
                                    rd = getServletContext().getRequestDispatcher(backUrl + terminalId + "&mid=" + merchantId);
                                } else {
                                    merchantId = sessionVarlist.getMerchantId();
                                    request.setAttribute("errorMsg", MessageVarList.TERMINAL_BLOCK_FAIL);
                                    rd = getServletContext().getRequestDispatcher(backUrl + terminalId + "&mid=" + merchantId);
                                }
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Blocked Terminal." + " Terminal ID : " + terminalId + "; by : " + sessionVarlist.getCMSSessionUser().getUserName());
            ...
            systemAuditBean.setPageCode(PageVarList.CALLCENTER_TERMINAL_BLOCK);
    ...
    public Boolean updateTerminalStatus(String terminalID) throws Exception {
        Boolean status = false;
        try {
            status = allocationmanager.updateTerminalStatus(terminalID, systemAuditBean, StatusVarList.TERMINAL_BLOCK, "3");
        } catch (Exception e) {
            throw e;
        }
        return status;

 ...
 * @author chanuka
 */
public class ManageUserRolePageServlet extends HttpServlet {
    ...
    private String url = "/administrator/controlpanel/systemusermgt/userrolepageassign.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String[] notToAssign = request.getParameterValues("notassignpagelist");
            String[] toAssign = request.getParameterValues("assignpagelist");
            ...
            if (toAssign != null && toAssign.length > 0) {
                ...
                newValue = userRoleCode + "|" + "Page|" + toAssignVal + sessionVarlist.getCMSSessionUser().getUserName();
            this.getPagebySectionAndUserRoleCode(userRoleCode, sectionCode);
                String oldAssignVal = "";
                for (PageBean bean : toPageList) {
                    oldAssignVal += bean.getDescription() + "|";
                }
                oldValue = userRoleCode + "|" + "Page|" + oldAssignVal + "|" + sessionVarlist.getCMSSessionUser().getUserName();
                
                
                this.setAudittraceValueForAdd(request);
                this.assignSectionPage(userRoleCode, sectionCode, toAssign, sessionUser.getUserName(), systemAuditBean);
            }

            if (notToAssign != null && notToAssign.length > 0) {
                this.setAudittraceValueForDelete(request);
                this.unAssignSectionPage(userRoleCode, sectionCode, notToAssign, systemAuditBean);
            }
            ...
            request.setAttribute("successMsg", MessageVarList.SUCCESS_ASSIGN_USER_SECTION_PAGES);
            ...
            request.setAttribute("errorMsg", MessageVarList.ERROR_USER_SECTION_PAGES);
     ...
     * @throws Exception 
     */
    private void assignSectionPage(String userRoleCode, String sectionCode, String[] assinArray, String userName, SystemAuditBean systemAuditBean) throws Exception {

        try {
            systemUserRoleManager = new SystemUserRoleManager();
            systemUserRoleManager.assignUserRoleSectionPages(userRoleCode, sectionCode, assinArray, userName, systemAuditBean);
        } catch (Exception ex) {
            throw ex;
        }
    }
     ...
     * @throws Exception 
     */
    private void unAssignSectionPage(String userRoleCode, String sectionCode, String[] assinArray, SystemAuditBean systemAuditBean) throws Exception {

        try {
            systemUserRoleManager = new SystemUserRoleManager();
            systemUserRoleManager.UnAssignUserRoleSectionPages(userRoleCode, sectionCode, assinArray, systemAuditBean);

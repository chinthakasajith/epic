 ...
 * @author upul
 */
public class AddUpdateSectionPageServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String application = request.getParameter("applicationcodefield").trim();
            section = request.getParameter("sectioncode").trim();
            ...
            } else if (section.contentEquals("") || section.length() <= 0) {

                isValid = false;
                errorMsg = MessageVarList.SECTION_NULL;
            ...
            if (isValid) {
                ...
                if (this.isValidTaskByUser(sessionVarlist.getUserPageTaskList(), taskCode)) {
                    ...
                    this.assignSectionPage(section, toAssign, notToAssign, sessionUser.getUserName(), systemAuditBean);
                    this.getPageListNotAssignBySection();
                    request.setAttribute("notassigned", remainingPageList);
                ...
                if (existPageList.size() > 0) {


                    String errMessage = "You cannot remove follwing pges. ";
                    request.setAttribute("errorMsg", errMessage);
                    rd = getServletContext().getRequestDispatcher("/ManageApplicationPageFormServlet");
                    rd.include(request, response);

                } else {
                    String successMessage = "Successfully assigns pages. ";
    ...
    private void assignSectionPage(String sectionCode, String[] assinArray, String[] UnAssinArray, String sysUser, SystemAuditBean systemAuditBean) throws Exception {

        try {

            applicationSectionPageManager = new ApplicationSectionPageManager();
            existPageList = applicationSectionPageManager.assignSectionPage(sectionCode, assinArray, UnAssinArray, sysUser, systemAuditBean);
        } catch (Exception ex) {
            throw ex;
        }
    }
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Add pages for section ,section code :" + section + " by " + sessionVarlist.getCMSSessionUser().getUserName());

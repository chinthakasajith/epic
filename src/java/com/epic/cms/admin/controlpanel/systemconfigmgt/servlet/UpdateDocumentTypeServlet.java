 ...
 * @author ayesh
 */
public class UpdateDocumentTypeServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            request.setAttribute(RequestVarList.DOCUMENT_TYPE_ALLLIST,
                    DocumentTypeMgtManager.getInstance().getAllCretidScoreFieldDetails());
            request.setAttribute(RequestVarList.DOCUMENT_CARD_CAT_LIST, DocumentTypeMgtManager.getInstance().getCardCategoryList());


            DocumentTypeMgtBean docBean = new DocumentTypeMgtBean();

            docBean.setDocCode(request.getParameter("docCode"));
            docBean.setDocName(request.getParameter("docName"));
            docBean.setVerifyCat(request.getParameter("verifiCat"));
            docBean.setPostFix(request.getParameter("postfix"));
            docBean.setStatus(request.getParameter("status"));
            docBean.setCardCategory(request.getParameter("cardCategoty"));
            if(request.getParameter("mandatory")!=null){
               docBean.setIsMandatory(Boolean.parseBoolean(request.getParameter("mandatory")));
            }
            
            docBean.setLastupdateUser(sessionVarlist.getCMSSessionUser().getUserName());

            request.setAttribute(RequestVarList.DOCUMENT_TYPE_BEAN, docBean);

            String oldVal = request.getParameter("oldvalue");
            String newVal = docBean.getDocCode() + "|" + docBean.getDocName() + "|" + docBean.getVerifyCat() + "|" + docBean.getPostFix() + "|" + docBean.getStatus()+ "|" + docBean.getCardCategory()+ "|" + docBean.getIsMandatory() ;
            
            int row = -1;
            if (DocumentTypeMgtManager.getInstance().isValidate(docBean)) {
                this.setAudittraceValue(request , oldVal , newVal);
                row = DocumentTypeMgtManager.getInstance().UpdateDocumentType(docBean, systemAuditBean);
            }
            if (row == 1) {
                request.setAttribute("operationtype", "default");
                request.setAttribute(MessageVarList.JSP_SUCCESS, MessageVarList.DOCUMENT_TYPE_UPDATE_SUCCESS);
            }
    ...
    private void setAudittraceValue(HttpServletRequest request , String oldValue , String newValue) throws Exception {
        ...
        try {
            ...
            String uniqueId = request.getParameter("docCode");


            systemAuditBean.setUserRoleCode(sessionVarlist.getCMSSessionUser().getUserRole());
            systemAuditBean.setUserName(sessionVarlist.getCMSSessionUser().getUserName());
            systemAuditBean.setUniqueId(uniqueId);
            systemAuditBean.setDescription("Update Document Type. Document Tyoe Code: '" + uniqueId + "' by: " + sessionVarlist.getCMSSessionUser().getUserName());
            systemAuditBean.setApplicationCode(ApplicationVarList.ADMINISTRATION_APPLICATION);
            systemAuditBean.setSectionCode(SectionVarList.SYSTEMCONFIGMANAGEMENT);
            systemAuditBean.setPageCode(PageVarList.DOCUMENTTYPEMGT);

 ...
 * @author badrika
 */
public class AddEmbossFileFormatMgtServlet extends HttpServlet {
        ...
        try {
            ...
            if (this.validateUserInput(embean, request)) {
                ...
                try {
                    ...
                    ctypeexist = verifyCType(ctype);
                    if (!ctypeexist) {
                        success = this.insertNewEmbossFileFormat(recordFormat, recLenth);
                    } else {
                        request.setAttribute(MessageVarList.JSP_ERROR, MessageVarList.CARD_TYPE_EXIST);
                    }
            ...
            if (success > 0) {
                ...
                request.setAttribute(MessageVarList.JSP_SUCCESS, MessageVarList.FORMAT_ADD_SUCCESS);
            }
            ...
            request.setAttribute(MessageVarList.JSP_ERROR, MessageVarList.RECORD_COUNT_EMPTY);
            rd = request.getRequestDispatcher(url);
        } finally {
    ...
    /**
     * to check the task is valid to the logged user 
     * @param userTaskList
     * @param task
     * @return
     * @throws Exception 
    ...
    private void setValuesToBean(HttpServletRequest request) throws Exception {
        try {
            ...
            embean.setCardType(request.getParameter("cardType"));
            ...
            if(!embean.getRecordCount().isEmpty()) {
            recordFormat = this.getRecordFormat();
  
            String rfnew = recordFormat.replace("|", ",");            
            
            newValue = embean.getCardType() + "|" + embean.getFormatCode() + "|" + embean.getDescription() + 
                    "|" + embean.getDescription() + "|" + embean.getRecordCount() + "|" + embean.getStatus()+ "|" + rfnew;
     ...
     * @throws Exception 
     */
    public int insertNewEmbossFileFormat(String recformat, String reclength) throws Exception {
        try {
            effmManager = new EmbossFileFormatMgtManager();
            int i = effmManager.addNewEmbossFileFormat(systemAuditBean, embean, recformat, reclength);
            return i;
        } catch (Exception e) {
            throw e;
        }
    ...
    /**
     * to get the record format using all fields and separators
     * @return 
     */
    ...
    /**
     * to verify the record is already exist
     * @param s
     * @return
     * @throws Exception 
     */
    public boolean verifyCType(String s) throws Exception {
        boolean flag = false;
        try {
            effmManager = new EmbossFileFormatMgtManager();
            flag = effmManager.verifyCType(s);
        } catch (Exception ex) {
            throw ex;
        }
        return flag;
    ...
    public boolean validateUserInput(EmbossFileFormatBean embean, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            if (embean.getCardType().isEmpty() || embean.getCardType().length() <= 0) {
                isValidate = false;
                errorMessage = MessageVarList.CARD_TYPE_EPMTY;
            } else if (embean.getFormatCode().isEmpty() || embean.getFormatCode().length() <= 0) {
                isValidate = false;
            ...
            } else if (embean.getFormatCode().isEmpty() || embean.getFormatCode().length() > 6) {
                isValidate = false;
                errorMessage = MessageVarList.FORMAT_CODE_LENGTH;
            } else if (embean.getDescription().isEmpty() || embean.getDescription().length() <= 0) {
                isValidate = false;
            ...
            } else if (embean.getDescription().isEmpty() || embean.getDescription().length() > 64) {
                isValidate = false;
                errorMessage = MessageVarList.FORMAT_DES_LENGTH;
            } else if (embean.getRecordCount().isEmpty() || embean.getRecordCount().length() <= 0) {
                isValidate = false;
                errorMessage = MessageVarList.RECORD_COUNT_EMPTY;

            } else if (embean.getStatus().isEmpty() || embean.getStatus().length() <= 0) {
                isValidate = false;
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            systemAuditBean.setDescription("Add File Format. Format Code: " + uniqueId + "; by:  " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(AddEmbossFileFormatMgtServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

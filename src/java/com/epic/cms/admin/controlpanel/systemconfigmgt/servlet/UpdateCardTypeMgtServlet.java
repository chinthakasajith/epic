 ...
 * @author ayesh
 */
public class UpdateCardTypeMgtServlet extends HttpServlet {
    ...
    private List<CardTypeMgtBean> cardTypeMgtBeanList;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            cardTypeMgtBeanList = CardTypeMgtManager.getInctance().getAllCardType();
            
            for (CardTypeMgtBean cardTypeMgtBean : cardTypeMgtBeanList) {
                if(cardTypeMgtBean.getCardTypeCode().equals(bean.getCardTypeCode())){
            oldValue = cardTypeMgtBean.getCardTypeCode() + "|" + cardTypeMgtBean.getDescription() + "|" + cardTypeMgtBean.getStatusDes() + "|" + sessionUser.getUserName();
                }
            }
            
            if (CardTypeMgtManager.getInctance().inValidToProcess(bean)) {
                this.setAudittraceValue(request);
                int row = CardTypeMgtManager.getInctance().UpdateCardType(systemAuditBean, bean);
                if (row == 1) {
                    request.setAttribute(MessageVarList.JSP_SUCCESS, MessageVarList.CARD_TYPE_MGT_UPDATE_SUCCESS);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            systemAuditBean.setDescription("Update Card Type. Card Type Code: '" + uniqueId + "' by: " + sessionVarlist.getCMSSessionUser().getUserName());

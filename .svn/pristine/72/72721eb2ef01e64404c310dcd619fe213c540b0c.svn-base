...
//    
    ...
    public boolean updateCardApplication(String applicationId, String sysRecoCredit, String sysRecCardproduct, String creditLimit, String cardProduct, String productionMode, String cardKey, String binProfile, String custemCode, String acctempCode, String cardtempCode, String currency, String cif) throws Exception {
        boolean status = false;
        try {
            sysAuditManager = new SystemAuditManager();
            checkingPersis = new AppilcationCheckingPersistance();
            CMSCon = DBConnection.getConnection();
            CMSCon.setAutoCommit(false);
            status = checkingPersis.updateCardApplication(CMSCon, applicationId, sysRecoCredit, sysRecCardproduct, creditLimit, cardProduct, productionMode, cardKey, binProfile, custemCode, acctempCode, cardtempCode, currency, cif);
    ...
    //get cif number to identification number(must be changed in future)
    public String getCifNumberToIdentificationNumber(String idNum) throws Exception {
        String cif = null;
        try {
            checkingPersis = new AppilcationCheckingPersistance();
            CMSCon = DBConnection.getConnection();
            CMSCon.setAutoCommit(false);
            cif = checkingPersis.getCifNumberToIdentificationNumber(CMSCon, idNum);
            CMSCon.commit();
        } catch (SQLException ex) {
            try {
                CMSCon.rollback();
    ...
    public List<OccupationBean> getOccupationlist() throws Exception {
        try {
            ...
            occupationList = checkingPersis.getAllOccupationLst(CMSCon);

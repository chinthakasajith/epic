 ...
 * @author asela
 */
public class UpdateAtmCardIMKSMIPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String imksmi = request.getParameter("imksmi");    
            
            if (imksmi.length() == 32) {
                if (UserInputValidator.isHexxx(imksmi)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.IMKSMI_PREFIX + imksmi).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setImksmi(imksmi);
                        keyBean.setImksmiKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateIMKSMIKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update IMKSMI and IMKSMI KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateIMKSMIKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateIMKSMIKeyDetails(keyBean, systemAuditBean);

 ...
 * @author janaka_h
 */
public class RemoveBankDataFromSessionServlet extends HttpServlet {
...
//
            ...
            for (int j = 0; j < sessionVarlist.getSessionBankDetailList().size(); j++) {
                if (sessionVarlist.getSessionBankDetailList().get(j).getBankCode().equals(request.getParameter("bank")) && sessionVarlist.getSessionBankDetailList().get(j).getAccountNumber().equals(request.getParameter("account"))) {
...
//            this.getAllDocumentDetails(appliactionId);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(RemoveBankDataFromSessionServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

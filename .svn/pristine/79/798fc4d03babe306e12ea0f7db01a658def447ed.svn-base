 ...
 * @author asela
 */
public class UpdateCorporateDataServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try {
                ...
                String pageCode = PageVarList.CORPORATEASSIGNDATAUPDATE;
                ...
                if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                    ...
                    if (this.validateUserInput(appAssignBean, request)) {
                        ...
                        if (this.updateAssignApplication(appAssignBean, systemAuditBean)) {
                            ...
                            rd = getServletContext().getRequestDispatcher("/LoadCorporateDataSearchServlet");
                        } else {
    ...
    public void setUserInputToBean(HttpServletRequest request) throws Exception {
        ...
        String identityType = request.getParameter("identificationType").trim();
...
//        String assignStatus = request.getParameter("assignstatus").trim();
...
//        appAssignBean.setAssignStatus(assignStatus);


        appAssignBean.setLastUpdatedUser(sessionUser.getUserName());
        appAssignBean.setLastUpdatedTime(new Date());

        newValue = appAssignBean.getApplicationDomain() + "|" + appAssignBean.getCardCategory() + "|" + appAssignBean.getAssignUser() + "|"
                + appAssignBean.getIdentityNo() + "|" + appAssignBean.getIdentityType() + "|" + appAssignBean.getPriorityLevel() + "|"
                + appAssignBean.getReferralBranchId() + "|" + appAssignBean.getReferralEmpNo() + "|" + appAssignBean.getLastUpdatedUser();


        this.searchAssignApplication(sessionVarlist.getSearchAssignAppBean());

        for (ApplicationAssignBean bean : searchList) {
            if (bean.getApplicationId().equals(appliactionId)) {

                oldValue = appAssignBean.getApplicationDomain() + "|" + appAssignBean.getCardCategory() + "|" + appAssignBean.getAssignUser() + "|"
                        + appAssignBean.getIdentityNo() + "|" + appAssignBean.getIdentityType() + "|" + appAssignBean.getPriorityLevel() + "|"
                        + appAssignBean.getReferralBranchId() + "|" + appAssignBean.getReferralEmpNo() + "|" + appAssignBean.getLastUpdatedUser();
    ...
    public boolean validateUserInput(ApplicationAssignBean assignBean, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //            } 
                ...
                isValid = false;
                errorMessage = MessageVarList.CORPORATEASSIGN_IDENTITY_NUMBER_NULL;
            } else if (assignBean.getPriorityLevel().contentEquals("") || assignBean.getPriorityLevel().length() <= 0) {
                isValid = false;
                errorMessage = MessageVarList.CORPORATEASSIGN_PRIORITY_NULL;
            } else if (assignBean.getAssignUser().contentEquals("") || assignBean.getAssignUser().length() <= 0) {
                isValid = false;
                errorMessage = MessageVarList.CORPORATEASSIGN_ASSIGNUSER_NULL;
    ...
    private void getAllUserList() throws Exception {
        try {
            ...
            usersList = appAssignManager.getAllUserList(PageVarList.CORPORATEASSIGNDATAUPDATE);
        } catch (Exception ex) {
            throw ex;
        }
    }
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            systemAuditBean.setPageCode(PageVarList.CORPORATEASSIGNDATAUPDATE);

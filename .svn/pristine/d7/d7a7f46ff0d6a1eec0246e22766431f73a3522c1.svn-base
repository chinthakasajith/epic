 ...
 * @author badrika
 */
public class ViewTempLimitIncrementServlet extends HttpServlet {
...
//    private PermLimitIncrementBean permBean2, updateBean;
    private TempLimitIncrementBean permBean2, updateBean;
//    private PermLimitIncrementManager permManager;
    private TempLimitIncrementManager tempManager;
...
//                permManager=new PermLimitIncrementManager();
                tempManager=new TempLimitIncrementManager();
                
                updateBean = tempManager.getOnlineDetails(cardNumber);
                
                updateBean.setLastUpdatedUser(sessionVarlist.getCMSSessionUser().getUserName());                
                boolean successupdate = tempManager.updateTempLimitIncDetails(updateBean, cardNumber);

                if (successupdate) {
                    for (TempLimitIncrementBean permBean1 : sessionVarlist.getTempBeanList()) {
                    ...
                    if (permBean2 != null) {
                        if (updateBean != null) {
                            ...
                            permBean2.setOnlineTempCrediAmt(updateBean.getOnlineTempCrediAmt());

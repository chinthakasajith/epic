    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (request.getParameter("view").equals("search")) {
                ...
                String amount = this.makeCurrencyFormat(confbean.getAmountD(), confbean.getExponent());
                confbean.setFormatedamount(amount); 
...
//                this.getDateFromCommunicationBean(txnBean);
    ...
    private void setDataToCommunicationBean(TransactionAdjustment bean) throws Exception {
        ...
        try {
            ...
            txnBean.setAMOUNT(bean.getFormatedamount());
...
//            txnBean.setTXNTYPE(TxnTypeCode.TXN_ADJUSTMENT);
    ...
    private String makeCurrencyFormat(Double amount, Double exp) throws Exception {

        try {

            String formatted = "";
            Double multiplied = amount * (Double) Math.pow(10, exp);
            int multipiedamount = multiplied.intValue();

            if (String.valueOf(multipiedamount).length() < 12) {
                formatted = String.format("%012d", multipiedamount);

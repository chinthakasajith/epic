 ...
 * @author asela
 */
public class UpdateAWKKeyServlet extends HttpServlet {
            ...
            if (awk.length() == 32) {
                if (UserInputValidator.isHexxx(awk)) {
                    ...
                    try {
                        ...
                        keyBean = new KeyBean();
                        keyBean.setAwk(awk);
                        keyBean.setAwkkvc(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateAWKKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    public boolean updateAWKKeyDetails(KeyBean keyBean) throws Exception {
        boolean success = false;
        try {
            keyManager = new KeyManager();
            success = keyManager.updateAWKKeyDetails(keyBean, systemAuditBean);

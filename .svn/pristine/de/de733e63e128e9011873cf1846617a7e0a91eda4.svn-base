 ...
 * @author asela
 */
public class UpdateCreditProprietaryAWKPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String awk = request.getParameter("awk");

            if (awk.length() == 32) {
                if (UserInputValidator.isHexxx(awk)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.AWK_PREFIX + awk).substring(4);
                        ...
                        if (chanelId == null) {
                            ...
                            if (keyMgtmanager.isChanelAvailable(chanelId)) {
                                keyMgtmanager.updateAWK(awk, KVC, chanelId, systemAuditBean);
                            } else {
                                keyMgtmanager.addAWK(awk, KVC, chanelId, systemAuditBean);
                            }
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CIRRUS AWK and AWK KVC by " + sessionVarlist.getCMSSessionUser().getUserName());

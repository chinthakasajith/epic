 ...
 * @author asela
 */
public class UpdateAtmCardICVKBPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String icvkb = request.getParameter("icvkb");    
            
            if (icvkb.length() == 16) {
                if (UserInputValidator.isHexxx(icvkb)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.ICVK_PREFIX + icvkb).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setIcvkb(icvkb);
                        keyBean.setIcvkbKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateICVKBKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update ICVKB and ICVKB KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateICVKBKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateICVKBKeyDetails(keyBean, systemAuditBean);

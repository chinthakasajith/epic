 ...
 * @author asela
 */
public class UpdateAtmCardICVKAPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String icvka = request.getParameter("icvka");    
            
            if (icvka.length() == 16) {
                if (UserInputValidator.isHexxx(icvka)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.ICVK_PREFIX + icvka).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setIcvka(icvka);
                        keyBean.setIcvkaKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateICVKAKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update ICVKA and ICVKA KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateICVKAKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateICVKAKeyDetails(keyBean, systemAuditBean);

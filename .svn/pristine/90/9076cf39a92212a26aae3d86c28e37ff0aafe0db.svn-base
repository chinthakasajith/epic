 ...
 * @author nalin
 */
public class DeleteResponceCodeMgtServlet extends HttpServlet {
        ...
        try {
            ...
            try {
                ...
                if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                    ...
                    String responceCode = request.getParameter("responceCode");
                    responceBean = new ResponceCodeBean();
                    responceBean.setResponceCode(responceCode);

                                responceManager = new ResponceCodeMgtManager();
                                List<ResponceCodeBean> responceList1 = new ArrayList<ResponceCodeBean>();

                                responceList1 = responceManager.getResponceCodes();
                                for (ResponceCodeBean newResponceBean : responceList1) {
                                    if (newResponceBean.getResponceCode().equals(responceBean.getResponceCode())) {
                                        oldValue = newResponceBean.getResponceCode() + "|" + newResponceBean.getDescription() + "|" + sessionUser.getUserName();
                                    }
                                }
                    ...
                    try {
                        successDelete = deleteResponceCode(responceBean, systemAuditBean);

                        if (successDelete) {

                            request.setAttribute("successMsg", MessageVarList.RESPONCE_CODE_SUCCESS_DELETE + " " + responceCode);
                        }
                    ...
                    rd = getServletContext().getRequestDispatcher("/LoadResponceCodeMgtServlet");
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            String uniqueId = request.getParameter(responceBean.getResponceCode());
            ...
            //set description 
            systemAuditBean.setDescription("Delete Responce Code. Responce Code :" + responceBean.getResponceCode() + "; by : " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean deleteResponceCode(ResponceCodeBean responceBean, SystemAuditBean systemAuditBean) throws Exception {
        ...
        try {
            responceManager = new ResponceCodeMgtManager();
            success = responceManager.deleteResponceCode(responceBean, systemAuditBean);

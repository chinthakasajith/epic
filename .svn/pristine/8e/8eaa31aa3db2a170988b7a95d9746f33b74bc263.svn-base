 ...
 * @author asela
 */
public class UpdateAtmCardPVKAPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String pvka = request.getParameter("pvka");    
            
            if (pvka.length() == 16) {
                if (UserInputValidator.isHexxx(pvka)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.PVK_PREFIX + pvka).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setPvka(pvka);
                        keyBean.setPvkaKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updatePVKAKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update PVKA and PVKA KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updatePVKAKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updatePVKAKeyDetails(keyBean, systemAuditBean);

 ...
 * @author asela
 */
public class UpdateZMKKeyServlet extends HttpServlet {
            ...
            if (zmk.length() == 32) {
                if (UserInputValidator.isHexxx(zmk)) {
                    ...
                    try {
                        ...
                        keyBean = new KeyBean();
                        keyBean.setZmk(zmk);
                        keyBean.setZmkkvc(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateZMKKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update ZMK and ZMK KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateZMKKeyDetails(KeyBean keyBean) throws Exception {
        boolean success = false;
        try {
            keyManager = new KeyManager();
            success = keyManager.updateZMKKeyDetails(keyBean, systemAuditBean);

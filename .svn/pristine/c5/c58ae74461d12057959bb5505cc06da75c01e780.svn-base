 ...
 * @author asela
 */
public class UpdateCreditProprietaryCVVAServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String cvva = request.getParameter("cvva");

            if (cvva.length() == 16) {
                if (UserInputValidator.isHexxx(cvva)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.CVVA_B_PREFIX + cvva).substring(4);
                        ...
                        if (chanelId == null) {
                            ...
                            if (keyMgtmanager.isChanelAvailable(chanelId)) {
                               keyMgtmanager.updateVISACVVA(cvva, KVC, chanelId, systemAuditBean);
                            } else {
                                keyMgtmanager.addVISACVVA(cvva, KVC, chanelId, systemAuditBean);
                            }
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CIRRUS CVVA and CVVA KVC by " + sessionVarlist.getCMSSessionUser().getUserName());

 ...
 * @author asela
 */
public class UpdateAtmCardPVKPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String pvk = request.getParameter("pvk");    
            
            if (pvk.length() == 32) {
                if (UserInputValidator.isHexxx(pvk)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.PVK_PREFIX + pvk).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setPvk(pvk);
                        keyBean.setPvkKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updatePVKKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update PVK and PVK KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updatePVKKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updatePVKKeyDetails(keyBean, systemAuditBean);

...
package com.epic.cms.backoffice.installement.btpayment.persistance;
 ...
 * @author sajith_g
 */
public class BTPaymentPlanPersistance {
     ...
     * @throws Exception
     */
    public boolean insertPaymentPlan(Connection con, BTPaymentPlanBean paymentplan) throws Exception {
        boolean success = false;
        PreparedStatement insertStat = null;
        try {
            insertStat = con.prepareStatement("INSERT INTO BTPAYMENTPLAN(PAYMENTPLANCODE,DESCRIPTION, "
                    + "DURATION,INTERESTRATE,STATUS,LASTUPDATEDUSER,LASTUPDATEDTIME,CREATEDTIME) "
                    + "VALUES(?,?,?,?,?,?,?,?) ");
     ...
     * @throws Exception
     */
    public List<BTPaymentPlanBean> getAllPaymentPlans(Connection con) throws Exception {
        ...
        try {
            getAllPaymentPlans = con.prepareStatement("SELECT PP.PAYMENTPLANCODE,PP.DESCRIPTION,PP.DURATION,PP.INTERESTRATE, "
                    + "PP.LASTUPDATEDUSER,PP.CREATEDTIME,PP.LASTUPDATEDTIME, AST.DESCRIPTION AS STDESCRIPTION,PP.STATUS "
                    + "FROM BTPAYMENTPLAN PP, STATUS AST WHERE PP.STATUS = AST.STATUSCODE");
            rs = getAllPaymentPlans.executeQuery();
            paymentPlanList = new ArrayList<BTPaymentPlanBean>();
            while (rs.next()) {

                BTPaymentPlanBean paymentPlan = new BTPaymentPlanBean();
                ...
                paymentPlan.setCreatedTime(rs.getDate("CREATEDTIME"));
     ...
     * @throws Exception
     */
    public List<BTPaymentPlanBean> getAllActivePaymentPlans(Connection con) throws Exception {
        PreparedStatement getAllPaymentPlans = null;
        try {
            getAllPaymentPlans = con.prepareStatement("SELECT PP.PAYMENTPLANCODE,PP.DESCRIPTION,PP.DURATION,PP.INTERESTRATE, "
                    + "PP.LASTUPDATEDUSER,PP.CREATEDTIME,PP.LASTUPDATEDTIME, AST.DESCRIPTION AS STDESCRIPTION,PP.STATUS "
                    + "FROM BTPAYMENTPLAN PP, STATUS AST WHERE PP.STATUS = AST.STATUSCODE AND PP.STATUS='ACT'");
            rs = getAllPaymentPlans.executeQuery();
            paymentPlanList = new ArrayList<BTPaymentPlanBean>();
            while (rs.next()) {
     ...
     * @throws Exception
     */
    public boolean deletePaymentPlan(Connection con, BTPaymentPlanBean paymentPlan) throws Exception {
        boolean success = false;
        PreparedStatement deleteStat = null;
        try {
            deleteStat = con.prepareStatement("DELETE BTPAYMENTPLAN WHERE PAYMENTPLANCODE=? ");
     ...
     * @throws Exception
     */
    public boolean updatePaymentPlan(Connection con, BTPaymentPlanBean paymentplan) throws Exception {
        boolean success = false;
        PreparedStatement updateStat = null;
        try {
            updateStat = con.prepareStatement("UPDATE BTPAYMENTPLAN SET DESCRIPTION=?, "

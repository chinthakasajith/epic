 ...
 * @author sajith
 */
public class DeleteSMSTemplateServlet extends HttpServlet {
    ...
    private String url = "/administrator/templatemgt/smstemplate/smshome.jsp";
        ...
        try {
            ...
            if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                ...
                if (templateCode != null) {
                    ...
                    this.searchSMSMgt();
                    for (SMSBean bean : searchList) {
                        if (bean.getTemplateCode().equals(templateCode)) {
                            oldValue = bean.getTemplateCode() + "|" + bean.getDescription() + "|" + bean.getStatus() + "|" +  bean.getMessageBody();
                        }
                    }

                    if (this.deleteEmailConf(templateCode) >= 0) {
                        rd = getServletContext().getRequestDispatcher("/LoadSMSTempMgtServlet");
                        request.setAttribute("successMsg", MessageVarList.SUCCESS_DELETE_SMSCONF);

                    } else {
                        rd = getServletContext().getRequestDispatcher(url);
                        request.setAttribute("errorMsg", MessageVarList.ERROR_DELETE_SMSCONF);
            ...
            request.setAttribute("errorMsg", MessageVarList.ERROR_DELETE_SMSCONFIGURATION);
    ...
    private int deleteEmailConf(String templateCode) throws Exception {
        ...
        try {
            ...
            success = domainManager.deleteSMSTemplate(templateCode);
        } catch (Exception ex) {
            throw ex;
        }
        return success;
    ...
    private void searchSMSMgt() throws Exception {

 ...
 * @author chinthaka_r
 */
public class LoadAssignUsersServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try {
                ...
                dataCaptureUserList = appAssignManager.getAssignUserList(PageVarList.USERDATACAPTURE, cat);
                JSONObject jCombo = new JSONObject();// if there are several combo box to load
                //load combo box data
                JSONObject jObject = new JSONObject();
                for (String key : dataCaptureUserList.keySet()) {
                    jObject.put(key, dataCaptureUserList.get(key));
                }
                jCombo.put("combo2", jObject);
                out.print(jCombo);
                // out.print(users);
            } catch (Exception e) {

 ...
 * @author admin
 */
@WebServlet(name = "DeleteMerchantPaymentCycleServlet", urlPatterns = {"/DeleteMerchantPaymentCycleServlet"})
public class DeleteMerchantPaymentCycleServlet extends HttpServlet {
        ...
        try {
            ...
            try {
                ...
                if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                    ...
                    for (MerchantPaymentCycleBean paymentBean : paymentList) {
                        if (paymentBean.getPaymentCycleCode().equals(id)) {
                            oldValue = paymentBean.getPaymentCycleCode() + " | " + paymentBean.getPaymentOption() + " | "
                    ...
                    try {
                        ...
                        boolean delResult = deletePaymentData(id, systemAuditBean);

                        //if the deletion is success, proceed
                        if (delResult) {
                            this.getAllPaymentData();
                            request.setAttribute("paymentList", paymentList);
                            request.setAttribute("successMsg", " " + MessageVarList.PAYMENT_CYCLE_MGT_PAYMENT_CODE_SUCCESS_DELETE + "Payment Cycle code " + id);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Deleted Merchant Payment Cycle Payment Cycle Code :" + uniqueId + "; By " + sessionVarlist.getCMSSessionUser().getUserName());
     ...
     * @throws Exception 
     */
    public boolean deletePaymentData(String id, SystemAuditBean systemAuditBean) throws Exception {
        boolean success = false;
        try {
            cycleMgr = new MerchantPaymentCycleManager();
            success = cycleMgr.deletePaymentData(id, systemAuditBean);

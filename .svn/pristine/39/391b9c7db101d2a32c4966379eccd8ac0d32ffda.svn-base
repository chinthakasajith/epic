...
public class UpdateBankDetailsServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            //set bank details input to bean
            ...
            if (invalidMsgBean == null) {
                if (sessionVarlist.getSessionBankDetailList() != null) {
                    ...
                    int isadd = manager.insertAccountDetailList(sessionVarlist.getSessionBankDetailList(), sessionVarlist.getApplicationId(), systemAuditBean, autoSettelmentDetailsBean);

                    if (isadd == 2) {
                        checkingManager = new ApplicationCheckingManager();
                        checkingManager.addRemarks(sessionVarlist.getApplicationId(), "4", remark, sessionUser.getUserName());
                        request.setAttribute("successMsg", "Sucssefully Updated bank details.");

                    } else {
                        request.setAttribute("errorMsg", "Updating  failed.");
                    }
                }
            } else {
...
//          
            ...
            if (sessionVarlist.getApplicationTypeCode().equals(StatusVarList.CARD_AGAINST_FD_CODE)) {
                ...
                LoadApplicationStatus.loadDefaultEstablishmentApplicationStatusInUpdate(sessionVarlist.getApplicationId(), sessionVarlist, request, Boolean.TRUE, null, 2, false);
            } else if (sessionVarlist.getApplicationTypeCode().equals(StatusVarList.CARD_CATEGORY_MAIN)) {
                this.loadDefaultApplicationStatus(sessionVarlist.getApplicationId(), request);
            }
            rd = getServletContext().getRequestDispatcher(url);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(UpdateBankDetailsServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

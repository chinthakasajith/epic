 ...
 * @author asela
 */
public class UpdateIWKKeyServlet extends HttpServlet {
            ...
            if (iwk.length() == 32) {
                if (UserInputValidator.isHexxx(iwk)) {
                    ...
                    try {
                        ...
                        keyBean = new KeyBean();
                        keyBean.setIwk(iwk);
                        keyBean.setIwkkvc(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateIWKKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update IWK and IWK KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateIWKKeyDetails(KeyBean keyBean) throws Exception {
        boolean success = false;
        try {
            keyManager = new KeyManager();
            success = keyManager.updateIWKKeyDetails(keyBean, systemAuditBean);

 ...
 * @author ayesh
 */
public class AddDocumentTypeMgtServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            newValue = docBean.getDocCode() + "|" + docBean.getDocName() + "|" + docBean.getVerifyCat() + "|" + docBean.getPostFix() + "|" + docBean.getStatus() + "|" + docBean.getLastupdateUser()+"|"+docBean.getCardCategory()+"|"+docBean.getIsMandatory();
            ...
            if (DocumentTypeMgtManager.getInstance().isValidate(docBean)) {
                ...
                row = DocumentTypeMgtManager.getInstance().addNewDocumentType(docBean, systemAuditBean);
            }
            if (row == 1) {
                request.setAttribute(RequestVarList.DOCUMENT_TYPE_BEAN, null);
                request.setAttribute(MessageVarList.JSP_SUCCESS, MessageVarList.DOCUMENT_TYPE_ADD_SUCCESS);
            }
            request.setAttribute(RequestVarList.DOCUMENT_TYPE_ALLLIST, DocumentTypeMgtManager.getInstance().getAllCretidScoreFieldDetails());
            request.setAttribute(RequestVarList.DOCUMENT_CARD_CAT_LIST, DocumentTypeMgtManager.getInstance().getCardCategoryList());
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            systemAuditBean.setDescription("Add Document Type. Document Type Code: '" + uniqueId + "' by: " + sessionVarlist.getCMSSessionUser().getUserName());

 ...
 * @author janaka_h
 */
public class RemoveUpdateIncomeDataToSessionServlet extends HttpServlet {
...
//
            ...
            for (int j = 0; j < sessionVarlist.getSessionIncomeList().size(); j++) {
                if (sessionVarlist.getSessionIncomeList().get(j).getIncomeCatogary().equals(request.getParameter("category"))) {
                    sessionVarlist.getSessionIncomeList().remove(j);
                    break;
                }
...
//            sessionObj.setAttribute(SessionVarName.SESSION_OBJ, sessionVarlist);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(RemoveUpdateIncomeDataToSessionServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

 ...
 * @author chanuka
 */
public class DebitProcessGenerateAllMasterCardServlet extends HttpServlet {
        ...
        try {
            ...
            try {
                ...
                if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                    ...
                    masterCardsToEmbossList = sessionVarlist.getMasterCardsToEmbossList();


                    if (masterCardsToEmbossList == null || masterCardsToEmbossList.isEmpty()) {
...
////                            this.getAllMASTERCardToEmboss(tmp, StatusVarList.DEBIT);
                            this.getAllVISACardToEmboss(tmp, StatusVarList.DEBIT);

                        } else {
////                            this.getAllMASTERCardToEmboss(sessionVarlist.getEmbossSearchBean(), StatusVarList.DEBIT);
...
////                        request.setAttribute("cardEmbossingMASTERList", cardEmbossingMASTERList);
                        ...
                        while (itr.hasNext()) {
                            ...
                            for (int i = 0; i < masterCardsToEmbossList.size(); i++) {


                                if (i == (masterCardsToEmbossList.size() - 1)) {

                                    cardsOfProduct += masterCardsToEmbossList.get(i);
                                ...
                                CardEmbossingCardDetailsBean masterProductCardBean = this.getVISACardProductToEmbossBean(masterCardsToEmbossList.get(i));

                                if (masterProductCardBean.getCardProduct().equals(me.getKey())) {
                                    ...
                                    for (int j = 0; j < formatedRecord.length; j++) {
                                        ...
                                        if (j % 2 == 0) {
                                            ...
                                            if (!embossDetailBean.getMatchedTable().equals("NO")) {
                                                ...
                                                valueFromTable = this.getTableFieldValue(embossDetailBean.getMatchedTable(), embossDetailBean.getMatchedField(), masterProductCardBean.getCardNumber());
...
//                        else {
//
//                            this.getAllMASTERCardToEmboss(sessionVarlist.getEmbossSearchBean());
//                            this.getAllVISACardToEmboss(sessionVarlist.getEmbossSearchBean());
//
//
//
//                            for (int idx = 0; idx < cardEmbossingMASTERList.size(); idx++) {
...
//
//
//                            request.setAttribute("errorMsg", MessageVarList.ERROR_SUPPLY_DETAIL);
//                            rd = getServletContext().getRequestDispatcher(url);
//                        }
                    }
...
////    }
    ...
    private void setAudittraceValue(HttpServletRequest request, String cardType, String cardProduct) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Generate all " + cardType + " " + cardProduct + " Emboss file by " + sessionVarlist.getCMSSessionUser().getUserName());

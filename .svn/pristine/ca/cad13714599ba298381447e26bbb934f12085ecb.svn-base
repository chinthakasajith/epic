 ...
 */
package com.epic.cms.switchcontrol.keymgt.creditproprietary.servlet;
 ...
 * @author asela
 */
public class UpdateCreditProprietaryPVVAPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String pvva = request.getParameter("pvva");

            if (pvva.length() == 16) {
                if (UserInputValidator.isHexxx(pvva)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.PVVA_B_PREFIX + pvva).substring(4);
                        keyMgtmanager = new KeyManagementManager();
                        this.setAudittraceValue(request);

                        String chanelId = keyMgtmanager.getChanelId(StatusVarList.CHANEL_TYPE_CREADIT_PROPR);

                        if (chanelId == null) {
                            out.print(MessageVarList.CHANEL_NOT_AVAILABLE);
                        }else{
                            if (keyMgtmanager.isChanelAvailable(chanelId)) {
                                keyMgtmanager.updateVISAPVVA(pvva, KVC, chanelId, systemAuditBean);
                            }else{
                                 keyMgtmanager.addVISAPVVA(pvva, KVC, chanelId, systemAuditBean);
                            }
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CIRRUS PVVA and PVVA KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
            ...
            systemAuditBean.setPageCode(PageVarList.CREDIT_PROPR_KEY_MGT);

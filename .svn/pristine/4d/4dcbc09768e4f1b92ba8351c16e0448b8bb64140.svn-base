...
import com.epic.cms.backoffice.installementpayment.businesslogic.EasyPaymentConfirmManager;
 ...
 * @author badrika
 */
public class SearchEasyPaymentRequestServlet extends HttpServlet {

    private SystemUserManager systemUserManager;
    private SessionVarList sessionVarlist;
    private SessionUser sessionUser;
    private RequestDispatcher rd;
    private String url = "/backoffice/installementpayment/approveeasypayment.jsp";
//    private PermLimitIncrementBean permBean;
    private EasyPaymentConfirmBean bean;
    private List<EasyPaymentConfirmBean> requestList;
    private String errorMessage;
    private EasyPaymentConfirmManager manager;
    private EasyPaymentConfirmBean permBean2;
        ...
        try {
            ...
            if (request.getParameter("view").equals("search")) {
                ...
                if (this.validateUserInput(bean)) {
                    ...
                    this.searchRequests();
                    sessionVarlist.setEasyreqBeanList(requestList);
...
//                String txnid = request.getParameter("txnid");
                ...
                for (EasyPaymentConfirmBean bean1 : sessionVarlist.getEasyreqBeanList()) {
                ...
                sessionVarlist.setEasyreqBean(permBean2);
                ...
                EasyPaymentConfirmBean confbean = sessionVarlist.getEasyreqBean();
...
//                this.getDateFromCommunicationBean(txnBean);
                if (txnBean.getRESPONSECODE().equals(StatusVarList.MANUAL_TXN_SUCCESS_STATUS)) {
                    ...
                    if (success) {
                        request.setAttribute("successMsg", MessageVarList.EASY_PAYMENT_APPROVED);
                    } else {
                        request.setAttribute("permBean2", confbean);
                        request.setAttribute("operationtype", "view");
                        request.setAttribute("errorMsg", MessageVarList.EASY_PAYMENT_FAILED);
                ...
                if (request.getParameter("remarks").isEmpty()) {
                    ...
                    if (success) {
                        request.setAttribute("successMsg", MessageVarList.EASY_PAYMENT_REJECTED);
                    } else {
                        request.setAttribute("permBean2", confbean);
                        request.setAttribute("operationtype", "view");
            ...
            rd = getServletContext().getRequestDispatcher("/LoadEasyPaymentConfirmationServlet");
    ...
    private void searchRequests() throws Exception {
        try {
            manager = new EasyPaymentConfirmManager();
    ...
    private boolean approveRequest(EasyPaymentConfirmBean confbean, SystemAuditBean systemAuditBean) throws Exception {
        boolean success = false;
        try {
            manager = new EasyPaymentConfirmManager();
    ...
    private boolean rejectRequest(EasyPaymentConfirmBean confbean, SystemAuditBean systemAuditBean) throws Exception {
        boolean success = false;
        try {
            manager = new EasyPaymentConfirmManager();
    ...
    private void setValuesToBean(HttpServletRequest request) throws Exception {
        try {
            bean = new EasyPaymentConfirmBean();
    ...
    private boolean validateUserInput(EasyPaymentConfirmBean bean) {
    ...
    private void setAudittraceValue(HttpServletRequest request, EasyPaymentConfirmBean confbean) throws Exception {
        ...
        try {
            ...
            systemAuditBean.setPageCode(PageVarList.APPROVE_EASY_PAYMENT);
    ...
    private void setAudittraceValue2(HttpServletRequest request, EasyPaymentConfirmBean confbean) throws Exception {
        ...
        try {
            ...
            systemAuditBean2.setPageCode(PageVarList.APPROVE_EASY_PAYMENT);
    ...
    private void setDataToCommunicationBean(EasyPaymentConfirmBean bean) throws Exception {

        try {
            txnBean = new TxnBean();

            txnBean.setCARDNUMBER(bean.getCardno());
            txnBean.setAMOUNT(bean.getTxnamountonline());
            System.out.println(" getTxnamount  : " + txnBean.getAMOUNT());

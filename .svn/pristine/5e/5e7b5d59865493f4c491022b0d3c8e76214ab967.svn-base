 ...
 * @author asela
 */
public class UpdateAtmCardPPKPanelServlet extends HttpServlet {
            ...
            if (ppk.length() == 32) {
                if (UserInputValidator.isHexxx(ppk)) {
                    ...
                    try {
                        ...
                        keyBean.setPpk(ppk);
                        keyBean.setPpkKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updatePPKKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update PPK and PPK KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updatePPKKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updatePPKKeyDetails(keyBean, systemAuditBean);

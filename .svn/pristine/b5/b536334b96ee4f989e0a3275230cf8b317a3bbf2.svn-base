...
// * @author Badrika
// */
//public class ConfirmTempLimitIncrementServlet {
//    
//}
 ...
 * @author badrika
 */
public class ConfirmTempLimitIncrementServlet extends HttpServlet {
    ...
    private String cardNumber, incType, amount, status, creditLimit, cashLimit, avlCreditLimit, avlCashLimit, tempCreditAmt, tempCashAmt, incordec;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try {
                ...
                incType = request.getParameter("creditOrCash");
                incordec = request.getParameter("incordec");
                ...
                tempCreditAmt = request.getParameter("onlineTempCrediAmt");
                ...
                if (status.equals(StatusVarList.ACTIVE_STATUS)) {
                    ...
                    permbean.setAmount(amount);
                    permbean.setCreditOrCash(incType);
                    permbean.setIncordec(incordec);
...
//                    boolean succes = this.limitIncrement();
                    ...
                    permbean = this.tempLimitUpdate(permbean, systemAuditBean, feeBean);

                    if (permbean.isFlag()) {
                        request.setAttribute("operationtype", "search");

                        if (permbean.getCreditOrCash().equals("CREDIT")) {

                            request.setAttribute("successMsg", MessageVarList.CREDIT_LIMIT_INCREMENT_SUCCESS);
                        }
                        if (permbean.getCreditOrCash().equals("CASH")) {
...
//                    }
                    ...
                    request.setAttribute("operationtype", "search");
                    request.setAttribute("errorMsg", MessageVarList.NO_LIMIT_INCREMENT_SUCCESS);
                    rd = getServletContext().getRequestDispatcher(url);
                }
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            systemAuditBean.setDescription("Approve Temporary Limit Increment. Card Number: " + uniqueId + "; by: " + sessionVarlist.getCMSSessionUser().getUserName());
            ...
            systemAuditBean.setTaskCode(TaskVarList.LIMITINCREMENT);
    ...
    private TempLimitIncrementBean tempLimitUpdate(TempLimitIncrementBean tempBean, SystemAuditBean systemAuditBean, FeeBean feeBean) throws Exception {

        try {

            incrementManager = new TempLimitIncrementManager();
            tempBean = incrementManager.tempLimitUpdate1(tempBean, systemAuditBean, feeBean);

 ...
 * @author badrika
 */
public class RemoveManagerFromSessionServlet extends HttpServlet {
...
//            incomeBean.setApplicationId(sessionVarlist.getApplicationId());
            ...
            for (int j = 0; j < sessionVarlist.getSessionManagerDetailList().size(); j++) {
                if (sessionVarlist.getSessionManagerDetailList().get(j).getFname().equals(request.getParameter("fname")) && sessionVarlist.getSessionManagerDetailList().get(j).getLname().equals(request.getParameter("lname")) && sessionVarlist.getSessionManagerDetailList().get(j).getOficePhone().equals(request.getParameter("phone"))) {
                    sessionVarlist.getSessionManagerDetailList().remove(j);
                    break;
                }
...
//                }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(RemoveManagerFromSessionServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

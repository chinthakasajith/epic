...
public class DeleteQueueServlet extends HttpServlet {
    ...
    private ArrayList<QueueBean> queues = null;
    QueueBean queueBean;
    private String url = "/LoadQueueManagementServlet";
        ...
        try {
            ...
            try {
                ...
                if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                    ...
                    try {
                        //call the delete Queue method
                        boolean delResult = deleteQueue(id, systemAuditBean);

                        //if the deletion is success, proceed
                        if (delResult) {
                            queues = getQueues();
                            request.setAttribute("ladders", queues);
                            request.setAttribute("successMsg", " " + "Successfully deleted Queue with code " + id);
    ...
    public boolean deleteQueue(String code, SystemAuditBean systemAuditBean) throws Exception {
        boolean success = false;

        queueManager = new QueueManager();
        success = queueManager.deleteQueue(code, systemAuditBean);
        return success;
    }

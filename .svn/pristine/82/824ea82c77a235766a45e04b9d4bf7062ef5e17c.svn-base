 ...
 * @author asela
 */
public class UpdateAtmCardCSCKPanelServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String csck = request.getParameter("csck");    
            
            if (csck.length() == 32) {
                if (UserInputValidator.isHexxx(csck)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.CSCK_PREFIX + csck).substring(4);                 
                        this.setAudittraceValue(request);

                        String id = request.getParameter("id");
                        keyBean = new CardKeybean();
                        keyBean.setCsck(csck);
                        keyBean.setCsckKVC(KVC);
                        keyBean.setId(id);

                        successUpdate = this.updateCSCKKeyDetails(keyBean);
                        if (successUpdate) {
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update CSCK and CSCK KVC by " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean updateCSCKKeyDetails(CardKeybean keyBean) throws Exception {
        boolean success = false;
        try {
            cardKeyManager = new CardKeyManager();
            success = cardKeyManager.updateCSCKKeyDetails(keyBean, systemAuditBean);

 ...
 * @author sajith_g
 */
public class UpdateLetterConfServlet extends HttpServlet {
        ...
        try {
            ...
            if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                ...
                if (this.validateUserInput(letterBean, request)) {
                    ...
                    if (this.updateLetter(letterBean) >= 0) {
                        rd = getServletContext().getRequestDispatcher("/LoadLetterConfTempMgtServlet");
                        request.setAttribute("successMsg", MessageVarList.SUCCESS_UPDATE_LETTERCONF_GENERAL);
                    } else {
                        rd = getServletContext().getRequestDispatcher(url);
                        request.setAttribute("errorMsg", MessageVarList.ERROR_UPDATE_LETTERCONF);
                    }
            ...
            rd = getServletContext().getRequestDispatcher("/administrator/templatemgt/lettertemplate/letterhome.jsp");
    ...
    public void setUserInputToBean(HttpServletRequest request) {
        ...
        letterBean.setLastUpdatedUser(sessionUser.getUserName());

        oldValue = request.getParameter("oldValue");
        newValue = letterBean.getTemplateCode() + "|" + letterBean.getDescription() + "|" + letterBean.getStatus() + "|" + letterBean.getTitle()+ "|"+letterBean.getBody()+"|" ;

    }

    public boolean validateUserInput(LetterBean domainBean, HttpServletRequest request) throws Exception {
    ...
    private int updateLetter(LetterBean domainBean) throws Exception {

        try {

            domainManager = new LetterConfMgtManager();
            success = domainManager.UpdateLetterConf(domainBean);
        } catch (Exception ex) {
            throw ex;
        }
        return success;

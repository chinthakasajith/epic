        ...
        try {
            ...
            try {
                ...
                if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                    ...
                    alphacode = request.getParameter("currencyCodeAlpha");
                    currencyCode = request.getParameter("currencyCode");
                    currencyBean = new CurrencyBean();
                    currencyBean.setCurrencyCode(currencyCode);
                    currencyBean.setCurrencyAlphaCode(alphacode);

                       

                        for (CurrencyBean bean : sessionVarlist.getCurrencyDetailList()) {

                            if (bean.getCurrencyCode().equals(currencyCode)) {
                                currency = bean;
                             oldValue = currency.getCurrencyCode() + "|" + currency.getCurrencyAlphaCode() + "|" + currency.getExponent()+ "|" +
                                     currency.getCurrencyDes() + "|" + currency.getStatus();                                
                            }
                        }                    
                    ...
                    try {
                        boolean delResult = deleteCurrency(currencyBean, systemAuditBean);

                        if (delResult) {
                            request.setAttribute("successMsg", MessageVarList.CURRENCY_SUCCESS_DELETE + " code: "+alphacode);
...
//        systemAuditBean.setUniqueId(uniqueId);
            //set description 
            systemAuditBean.setDescription("Delete Currency. Currency Code: " + currencyCode + "; by: " + sessionVarlist.getCMSSessionUser().getUserName());
    ...
    public boolean deleteCurrency(CurrencyBean currency, SystemAuditBean systemAuditBean) throws Exception {
        ...
        try {
            ...
            success = currencyManager.deleteCurrency(currency, systemAuditBean);

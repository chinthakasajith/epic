 ...
 * @author janaka_h
 */
public class UpdateDocumentUploadServlet extends HttpServlet {
        ...
        try {
            ...
            if (sessionVarlist.getSessionDocumentList() != null) {
                ...
                if (isadd == 1) {
                    ...
                    checkingManager.addRemarks(sessionVarlist.getApplicationId(), "5", remark, sessionUser.getUserName());
                    request.setAttribute("successMsg", "Sucssefully uploaded documents.");

                } else {
                    request.setAttribute("errorMsg", "Uploading  failed.");
                }
            }

            if (sessionVarlist.getApplicationTypeCode().equals(StatusVarList.CARD_AGAINST_FD_CODE)) {
                LoadApplicationStatus.loadDefaultCardagainstFDApplicationStatusInUpdate(sessionVarlist.getApplicationId(), sessionVarlist, request, Boolean.TRUE, 3, false);

            } else if (sessionVarlist.getApplicationTypeCode().equals(StatusVarList.CARD_CATEGORY_COPORATE)) {
                LoadApplicationStatus.loadDefaultCorporateApplicationStatusInUpdate(sessionVarlist.getApplicationId(), sessionVarlist, request, Boolean.TRUE, 2, false);

            } else if (sessionVarlist.getApplicationTypeCode().equals(StatusVarList.CARD_CATEGORY_ESTABLISHMENT)) {
                LoadApplicationStatus.loadDefaultEstablishmentApplicationStatusInUpdate(sessionVarlist.getApplicationId(), sessionVarlist, request, Boolean.TRUE, null, 3, false);
...
//             request.setAttribute("errorMsg", "Error in action");
            if (sessionVarlist.getApplicationTypeCode().equals(StatusVarList.CARD_AGAINST_FD_CODE)) {
                ...
                LoadApplicationStatus.loadDefaultEstablishmentApplicationStatusInUpdate(sessionVarlist.getApplicationId(), sessionVarlist, request, Boolean.TRUE, null, 4, false);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(UpdateDocumentUploadServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

 ...
 * @author mahesh_m
 */
public class UpdateAmexCSKAServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String cska = request.getParameter("cska");

            if (cska.length() == 16) {
                if (UserInputValidator.isHexxx(cska)) {
                    securityModule = new ConnectionToSecurityModule();
                    try {
                        String KVC = securityModule.getServerMsg(StatusVarList.CSCK_PREFIX + cska).substring(4);
                        ...
                        if (chanelId == null) {
                            ...
                            if (keyMgtmanager.isChanelAvailable(chanelId)) {
                                keyMgtmanager.updateCSKA(cska, KVC, chanelId, systemAuditBean);
                            } else {
                                keyMgtmanager.addCSKA(cska, KVC, chanelId, systemAuditBean);
                            }
...
//                rd.forward(request, response);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update AMEX CSKA and KVC by " + sessionVarlist.getCMSSessionUser().getUserName());

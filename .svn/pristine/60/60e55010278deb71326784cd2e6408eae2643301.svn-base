...
public class ViewCustomerMgtServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
          ...
          //  String viewId=sessionVarlist.getCardNumber();
            ...
            cardCategoryCode=request.getParameter("cardCategoryCode");
            sessionVarlist.setCallcenterSearchCardCategoty(cardCategoryCode);
            this.searchCustomer();
            ...
            for (ViewDataBean CMSViewDataBean : cusList) {
                if (CMSViewDataBean.getCustmerIdCC().equals(viewId)) {
                    ...
                    sessionVarlist.setCallcenterSearchAppId(cusBean.getAppId());
                }
            }
            ...
            for (ViewDataBean CMSViewDataBean : appList) {
                ...
                if (cusBean !=null && cusBean.getAppId()!=null 
                        &&  CMSViewDataBean.getAppId().equals(cusBean.getAppId())) {
                    sessionVarlist.setCallcenterSearchCardCategoty(CMSViewDataBean.getCardcategory());
    ...
    private void searchCustomer() throws Exception {
        try {
            ...
            if(cardCategoryCode !=null && (cardCategoryCode.equals(StatusVarList.CARD_CATEGORY_SUPPLEMENTARY)
                    || cardCategoryCode.equals(StatusVarList.CARD_CATEGORY_ESTABLISHMENT))){
                cusList=callCenterMgtManager.searchCustomerwithCategory(searchbean ,cardCategoryCode);  
            }
    ...
    private void trackCardNumber(List<ViewDataBean> cardList) {
        String cardNumber="";
        
        if(cardList!=null && !cardList.isEmpty()){
            if(cardList.get(0).getMainCardNumber() !=null && !cardList.get(0).getMainCardNumber().trim().isEmpty()){
                cardNumber=cardList.get(0).getMainCardNumber();
            }else{
                cardNumber=cardList.get(0).getCardNumber();
            } 
        }       
        sessionVarlist.setCallcenterSearchCardCardNo(cardNumber);
    }
}

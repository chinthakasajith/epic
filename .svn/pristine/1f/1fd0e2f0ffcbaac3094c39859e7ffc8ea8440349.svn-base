 ...
 * @author ruwan_e
 */
public class AddQueueServlet extends HttpServlet {
            ...
            try {
                ...
                if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                    ...
                    if (validateUserInput(queueBean)) {
                        ...
                        if (insertQueue(queueBean)) {
                            out.print("success," + MessageVarList.QUEUE_MGT_SUCCESS_ADD + " Queue Code " + queueBean.getCode());
                        }
                    } else {
    ...
    private Map<String, String> getStatus() throws Exception {
        try {
            return new QueueManager().getStatus();
        } catch (Exception ex) {
            throw ex;
        }
    }
    ...
    private ArrayList<CaseTypeBean> getCases() throws Exception {
        try {
            return new QueueManager().getAllCases();
        } catch (Exception ex) {
            throw ex;
        }
    }
...
//                errorMessage = MessageVarList.LADDER_CARD_TYPE_EMPTY;
    ...
    public boolean insertQueue(QueueBean queue) throws Exception {
        boolean success = false;
        try {
            queueManager = new QueueManager();
            success = queueManager.create(queue, systemAuditBean);

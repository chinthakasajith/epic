 ...
 * @author chanuka
 */
public class UpdateCardBinServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try {
                ...
                String pageCode = PageVarList.CARDPBIN;
            ...
            if (this.validateUserInput(cardBinBean)) {
                ...
                String productCode = getProductCode(cardBinBean.getBinNumber());

                if (cardBinBean.getStatus().equals(StatusVarList.DEACTIVE_STATUS)) {
                    if (productCode.equals("")) {
                        request.setAttribute("operationtype", "add");
                        this.setAudittraceValue(request, oldVal, newVal);
                        this.updateCardBin(cardBinBean, systemAuditBean);
                        request.setAttribute("successMsg", cardBinBean.getBinNumber() + " " + MessageVarList.CARD_BIN_SUCCESS_UPDATE);
                        rd = getServletContext().getRequestDispatcher("/LoadCardBinServlet");


                    } else {
                        request.setAttribute("operationtype", "update");
                        request.setAttribute("cardBinBean", cardBinBean);
                        request.setAttribute("errorMsg", "This Bin can not be deactivated, used in " + productCode + " card product");
            ...
            request.setAttribute("errorMsg", MessageVarList.ERROR_UPDATE_CARD_BIN);
    ...
    private void getAllCardBinDetailsList() throws Exception {
        try {

            cardBinMgtManager = new CardBinMgtManager();
            cardBinBeanLst = cardBinMgtManager.getAllCardBinDetailsList();
    ...
    public boolean updateCardBin(CardBinBean cardBinBean, SystemAuditBean systemAuditBean) throws Exception {
        ...
        try {
            cardBinMgtManager = new CardBinMgtManager();
            success = cardBinMgtManager.updateCardBin(cardBinBean, systemAuditBean);
        } catch (Exception ex) {
            throw ex;
        }
        return success;
    ...
    public void setUserInputToBean(HttpServletRequest request) {
        ...
        oldVal = request.getParameter("oldvalue");
        ...
        cardBinBean.setProductionMode(request.getParameter("promode"));
        cardBinBean.setStatus(request.getParameter("status"));

        cardBinBean.setLastUpdateDate(new Date());
        cardBinBean.setLastupdateUser(sessionUser.getUserName());

        newVal = cardBinBean.getBinNumber() + "|" + cardBinBean.getDescription() + "|" + cardBinBean.getOnus() + "|" + cardBinBean.getCardType() + "|"
                + cardBinBean.getBinType() + "|" + cardBinBean.getCurrTypeCode() + "|" + cardBinBean.getProductionMode() + "|" + cardBinBean.getStatus() + "|"
    ...
    public boolean validateUserInput(CardBinBean cardBinBean) throws Exception {
        ...
        try {
            ...
            } else if (cardBinBean.getProductionMode().contentEquals("") || cardBinBean.getProductionMode().length() <= 0) {
                isValidate = false;
                errorMessage = MessageVarList.CARD_PRODMODE_NULL;
    ...
    private void setAudittraceValue(HttpServletRequest request, String oldV, String newV) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Update Issuing Bin. Bin Number: " + cardBinBean.getBinNumber() + "; by: " + sessionVarlist.getCMSSessionUser().getUserName());
            systemAuditBean.setApplicationCode(ApplicationVarList.ADMINISTRATION_APPLICATION);
            systemAuditBean.setSectionCode(SectionVarList.SYSTEMCONFIGMANAGEMENT);
            systemAuditBean.setPageCode(PageVarList.CARDPBIN);

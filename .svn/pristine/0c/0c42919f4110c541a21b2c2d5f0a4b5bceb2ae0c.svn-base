 ...
 * @author asela
 */
public class DeleteCommunicationKeyServlet extends HttpServlet {
            ...
            try {
                ...
                try {
                    ...
                    if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                        ...
                        try {
                            deleteStatus = this.deleteCommunicationKeyDetails(id);

                            if (deleteStatus >= 0) {
                                request.setAttribute("successMsg", MessageVarList.CARD_KEY_DELETE_SUCCESS);
                            } else {
                                request.setAttribute("errorMsg", MessageVarList.CARD_KEY_DELETE_SUCCESS);
                            }
    ...
    private int deleteCommunicationKeyDetails(String keyId) throws Exception {
        int num = -1;
        try {
            keyManager = new KeyManager();
            num = keyManager.deleteCommunicationKeyDetails(keyId,systemAuditBean);
            return num;
        } catch (Exception ex) {
            throw ex;
        }
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Delete Communication Keys. Key Id : '" + uniqueId + "' by: " + sessionVarlist.getCMSSessionUser().getUserName());

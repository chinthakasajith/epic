 ...
 * @author ayesh
 */
public class DeleteCardTypeMgtServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            for (CardTypeMgtBean cardTypeMgtBean : cardTypeMgtBeanList) {
                if(cardTypeMgtBean.getCardTypeCode().equals(id)){
            oldValue = cardTypeMgtBean.getCardTypeCode() + "|" + cardTypeMgtBean.getDescription() + "|" + cardTypeMgtBean.getStatus() + "|" + sessionUser.getUserName();
                }
            }            
            
            this.setAudittraceValue(request);
            int row = CardTypeMgtManager.getInctance().deleteCardtype(id, systemAuditBean);

            if (row == 1) {
                request.setAttribute(MessageVarList.JSP_SUCCESS, MessageVarList.CARD_TYPE_MGT_DELETE_SUCCESS);
            }
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            systemAuditBean.setDescription("Delete Card Type. Card Type Code: '" + uniqueId + "' by: " + sessionVarlist.getCMSSessionUser().getUserName());

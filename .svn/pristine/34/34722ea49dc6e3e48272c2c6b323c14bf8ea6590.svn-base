 ...
 * @author upul
 */
public class DeleteSingleHolidayServlet extends HttpServlet {
        ...
        try {
            ...
            if (this.isValidTaskByUser(sessionVarlist.getUserPageTaskList(), taskCode)) {
                ...
                if (this.validateholidayValues(holidayManagementBean)) {
                    ...
                    if (this.deleteHoliday(holidayManagementBean)) {

                        this.getHolidayListForMonth();
                        request.setAttribute("holidayList", hmbs);
                        request.setAttribute("tstVal", "val");
                        request.setAttribute(MessageVarList.JSP_SUCCESS, MessageVarList.DELETE_SUCCESS_HOLIDAY);
                        ...
                        request.setAttribute(MessageVarList.JSP_ERROR, MessageVarList.DELETE_ERROR_HOLIDAY);
                        //response.sendRedirect(request.getContextPath() + "/controlpanel/systemusermgt/viewsectionpage.jsp");
                        rd = getServletContext().getRequestDispatcher("/administrator/controlpanel/systemconfigmgt/holidaymanagementhome.jsp");
                        rd.forward(request, response);
    ...
    private void setholidayValuesToBean(HttpServletRequest request) throws Exception {
        try {
            ...
            oldValue = holidayManagementBean.getHolidayDate() + "|" + holidayManagementBean.getHolidayReason();
     ...
     * @throws Exception 
     */
    public boolean deleteHoliday(HolidayManagementBean managementBean) throws Exception {
        boolean success = false;
        try {

            holidayManagementManager = new HolidayManagementManager();
            success = holidayManagementManager.deleteHoliday(managementBean, systemAuditBean);
    ...
    private void setAudittraceValue(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            //set description 
            systemAuditBean.setDescription("Delete Holiday Managment. Date :" + uniqueId + "; by :" + sessionVarlist.getCMSSessionUser().getUserName());

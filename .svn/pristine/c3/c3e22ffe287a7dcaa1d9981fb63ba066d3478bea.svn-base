 ...
 * @author mahesh_m
 */
public class SearchDebitApplicationApproveServlet extends HttpServlet {
    ...
    private String url = "/camm/applicationconfirmation/debitcardapplicationsearch.jsp";
            ...
            try {
                ...
                if (this.isValidAccess(sessionUser.getUserRole(), pageCode, taskCode)) {
                    ...
                    if (this.validateUserInput(searchBean, request)) {
                        ...
                        this.searchDebitApplication(searchBean);
...
//            rd.forward(request, response);
    ...
    public void setUserInputToBean(HttpServletRequest request) {
        ...
        String idNum = request.getParameter("idNumber").trim();
        ...
        searchBean.setIdNumber(idNum);

        if (appType != null) {
            searchBean.setApplicationType(appType);
        } else {
            searchBean.setApplicationType("");
        }
    ...
    public boolean validateUserInput(SearchAssignAppBean searchBean, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            }else if (!UserInputValidator.isCorrectString(searchBean.getIdNumber())) {
                isValid = false;
                errorMessage = MessageVarList.APPASSIGN_IDNUM_INVALID;
            }
    ...
    private void searchDebitApplication(SearchAssignAppBean searchBean) throws Exception {

        try {

            confirmationManager = new ApplicationConfirmationManager();
            searchList = confirmationManager.debitCardGeneralSearch(searchBean);
    ...
    private void getAllUserList() throws Exception {
        try {
            ...
            usersList = appAssignManager.getAllUserList(PageVarList.DEBITAPPLICATONAPPROVE);
        } catch (Exception ex) {
            throw ex;
        }
    }
